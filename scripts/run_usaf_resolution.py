...
def _draw_chart_panel(ax, r: ChannelResult, *, margin_frac: float = 0.06) -> None:
    ...
    ax.add_patch(Rectangle((x0 - cx0, y0 - cy0), w_box, h_box,
    ...
    ax.annotate("H", xy=(crop_w - 4, y_h), xycoords="data",
                color="white", fontsize=9, fontweight="bold",
                ha="right", va="center",
                bbox=dict(facecolor=DIRECTION_COLORS["H"], alpha=0.95,
    ...
    ax.annotate("V", xy=(x_v, crop_h - 4), xycoords="data",
                color="white", fontsize=9, fontweight="bold",
                ha="center", va="bottom",
                bbox=dict(facecolor=DIRECTION_COLORS["V"], alpha=0.95,
                          edgecolor="none", boxstyle="round,pad=0.18"))
...
def _plot_direction_curve(ax, dr: DirectionResult, color: str,
                          linestyle: str, label_prefix: str,
                          show_cutoff_text: bool = True,
                          text_y_anchor: float = 0.97) -> None:
    f = dr.freqs
    m = f > 0
    ax.semilogy(f[m], dr.spectrum_smoothed[m], color=color,
                linewidth=1.6, linestyle=linestyle,
    ...
    for key, alpha in cutoff_styles:
        ...
        if fc is None:
            continue
        ax.axvline(fc, color=color, linestyle=linestyle,
    ...
    if show_cutoff_text and cutoff_lines:
        ...
        ax.text(0.98, text_y_anchor, text, transform=ax.transAxes,
                ha="right", va="top", fontsize=8, color=color,
                bbox=dict(facecolor="white", alpha=0.85,
                          edgecolor=color, linewidth=0.6, pad=2.5))


def plot_panels(gain: str, results: Dict[str, ChannelResult],
                out_path: Path, file_label: str) -> None:
    fig, axes = plt.subplots(2, 3, figsize=(13.5, 8.0),
                             gridspec_kw={"height_ratios": [1.05, 0.85]})
    ...
    for col, ch in enumerate(("R", "G", "B")):
        ...
        ax_sp = axes[1, col]
        _plot_direction_curve(ax_sp, r.horizontal,
                              color=CHANNEL_COLORS[ch],
                              linestyle="-", label_prefix=ch,
                              text_y_anchor=0.97)
        _plot_direction_curve(ax_sp, r.vertical,
                              color=CHANNEL_COLORS[ch],
                              linestyle="--", label_prefix=ch,
                              text_y_anchor=0.62)

        for thr, ls in [(0.5, "-"), (0.1, ":")]:
            ax_sp.axhline(thr, color="0.55", linestyle=ls, linewidth=0.6,
                          alpha=0.7)
        ...
        leg_handles = [
            Line2D([0], [0], color=CHANNEL_COLORS[ch], linestyle="-",
                   linewidth=1.6, label="H slice"),
            Line2D([0], [0], color=CHANNEL_COLORS[ch], linestyle="--",
                   linewidth=1.6, label="V slice"),
        ]
        ax_sp.legend(handles=leg_handles, loc="lower left", fontsize=8)

    fig.suptitle(
        f"{file_label} — {gain} gain — RGB lateral resolution",
    ...
    fig.text(
        ...
        ha="center", va="top", fontsize=8.5, color="0.25", wrap=True,
    )
...
def plot_overlay(gain: str, results: Dict[str, ChannelResult],
                 out_path: Path, file_label: str) -> None:
    fig, axes = plt.subplots(1, 2, figsize=(12.5, 4.5), sharey=True)
    for ax, direction, title in zip(axes, ("H", "V"),
                                    ("H slice (probes vertical bars)",
                                     "V slice (probes horizontal bars)")):
        for ch in ("R", "G", "B"):
            r = results[ch]
            dr = r.horizontal if direction == "H" else r.vertical
            f = dr.freqs
            m = f > 0
            ax.semilogy(f[m], dr.spectrum_smoothed[m],
                        color=CHANNEL_COLORS[ch], linewidth=1.6, label=ch)
            for key, ls, alpha in [("MTF50", "-", 0.45),
                                   ("MTF10", ":", 0.35)]:
                fc = dr.cutoffs.get(key)
                if fc is None:
                    continue
                ax.axvline(fc, color=CHANNEL_COLORS[ch], linestyle=ls,
                           linewidth=0.8, alpha=alpha)
        for thr, ls in [(0.5, "-"), (0.1, ":")]:
            ax.axhline(thr, color="0.55", linestyle=ls, linewidth=0.6,
                       alpha=0.6)
        ax.set_xlim(0, 0.5)
    ...
    fig.suptitle(f"{file_label} — {gain} gain — channel comparison",
...
def plot_hg_vs_lg(all_results: Dict[str, Dict[str, ChannelResult]],
                  out_path: Path, file_label: str) -> None:
    fig, axes = plt.subplots(2, 3, figsize=(12.5, 6.5), sharey="row",
                             gridspec_kw={"hspace": 0.45})
    metrics = [("MTF50", "MTF50  (50% peak)"),
               ("MTF20", "MTF20  (20% peak)"),
               ("MTF10", "MTF10  (10% peak ≈ Rayleigh)")]
    channels = ("R", "G", "B")
    width = 0.35
    x = np.arange(len(channels))
    for row_i, (direction, dlabel) in enumerate((("H", "H slice"),
                                                 ("V", "V slice"))):
        for col, (key, title) in enumerate(metrics):
            ...
            for i, gain in enumerate(("HG", "LG")):
                vals = []
                for ch in channels:
                    dr = (all_results[gain][ch].horizontal if direction == "H"
    ...
    fig.suptitle(f"{file_label} — HG vs LG detection-limit comparison",
...
def main(h5_path: str, out_dir: Path) -> None:
    ...
    print(f"  {rec.attrs.get('nickname', '?')}  "
    ...
    print(f"  wrote fig_HG_vs_LG_summary.png")

    print("\nDetection-limit frequencies (cycles / channel-pixel):")
    print(f"  {'gain':<4} {'ch':<2} {'dir':<3}  "
    ...
    for gain in ("HG", "LG"):
        for ch in ("R", "G", "B"):
            ...
            for direction, dr in (("H", r.horizontal), ("V", r.vertical)):
                ...
                print(f"  {gain:<4} {ch:<2} {direction:<3}  "

...
LIGHT = {
    ...
    "text": "#14181f", "text_muted": "#5d6773",
    "grid": "#e4e7ec", "table_alt": "#f6f7f9",
    "ok": "#1a7f37", "warn": "#b87b00", "fail": "#cf222e", "nyq": "#d97706",
}
DARK = {
    "fig_face": "#181b21", "ax_face": "#1e2128",
    "text": "#e8eaed", "text_muted": "#9aa3af",
    "grid": "#272b33", "table_alt": "#23262d",
    "ok": "#3fb950", "warn": "#e5a13a", "fail": "#ff6b6b", "nyq": "#d97706",
}
...
def _stretch(img: np.ndarray, lo_pct: float = 1.0, hi_pct: float = 99.5
             ) -> Tuple[float, float]:
...
def build_mtf_curves_fig(
    ...
    *, threshold: float = 0.3, theme: str = "light",
) -> Figure:
    th = _theme(theme)
    ...
    lim_per_channel: Dict[str, Optional[float]] = {}
    all_lp = []
    for ch, meas in channel_measurements.items():
        ...
        for direction, style in DIR_STYLES.items():
            ...
            all_lp.extend(xs)
            ax.plot(xs, ys, color=color, linewidth=1.3, alpha=0.8,
                    linestyle=style["linestyle"])
            ax.scatter(xs, ys, color=color, marker=style["marker"],
                       s=style["markersize"] ** 2, edgecolor="white",
    ...
    ax.text(ax.get_xlim()[1] if not all_lp else max(all_lp) * 1.05,
            threshold, f"  threshold  {int(threshold * 100)}%",
            va="center", ha="left", color=th["text_muted"], fontsize=9)
    ...
    ax.set_title("USAF MTF — Michelson vs spatial frequency",
                 fontsize=12.5, pad=14, loc="left", color=th["text"])
    fig.text(0.5, 0.945, "detection limit @ threshold  ·  " + "  ·  ".join(parts),
             ha="center", va="bottom", fontsize=9.5, color=th["text_muted"])

    ax.legend(loc="upper right", fontsize=8.5, labelcolor=th["text"],
              facecolor=th["ax_face"], edgecolor=th["grid"], framealpha=0.9,
...
def build_profile_gallery_fig(
    ...
    *, threshold: float = 0.3, theme: str = "light",
    ...
    if n == 0:
        ...
        ax = fig.add_subplot(111)
        ax.text(0.5, 0.5, "No lines picked.", ha="center", va="center",
                color=th["text_muted"], fontsize=11)
        ax.set_axis_off()
        return fig
    ...
    for i, spec in enumerate(specs):
        ...
        ax.set_title(
            ...
            fontsize=10, loc="left", color=th["text"])
        if m0 is not None:
            ...
            ax.text(0.02, 0.02, "  ".join(parts[:4]),
                    transform=ax.transAxes, fontsize=8, color=th["text_muted"],
                    ha="left", va="bottom",
                    family="monospace")
        ax.set_xlabel("distance along line (px)", fontsize=9)
        ax.set_ylabel("DN", fontsize=9)

    fig.suptitle("Profile gallery — extracted intensity along each picked line",
                 fontsize=12, color=th["text"], y=0.995)
...
def build_summary_table_fig(
    ...
    *, threshold: float = 0.3, theme: str = "light",
    ...
    headers = ["Channel", "G", "E", "D", "lp/mm", "Mich (P)", "Mich (FFT)",
               "Mich (min/max)", "s/cyc", "Reliability"]
    cell_rows = []
    row_colors = []
    for ch, spec, m in rows:
        cell_rows.append([
            ch,
            str(spec.group), str(spec.element), spec.direction,
            f"{m.lp_mm:.2f}",
    ...
    fig.suptitle(f"Summary table — {len(cell_rows)} measurement"
                 f"{'s' if len(cell_rows) != 1 else ''} "
                 f"· threshold {int(threshold * 100)}%",
                 fontsize=12, color=th["text"], y=0.985)
    return fig
...
def build_detection_overlay_fig(
    ...
    *, threshold: float = 0.3, theme: str = "light",
    ...
    if n == 0 or not specs:
        ...
        ax = fig.add_subplot(111)
        ax.text(0.5, 0.5, "Nothing to overlay.", ha="center", va="center",
                color=th["text_muted"], fontsize=11)
        ax.set_axis_off()
        return fig
    ...
    for idx, ch in enumerate(channels):
        ...
        for spec, m in zip(specs, meas):
            ...
            ax.plot([spec.p0[0], spec.p1[0]], [spec.p0[1], spec.p1[1]],
            ...
            ax.text(mx, my - 10,
                    ...
                    bbox=dict(boxstyle="round,pad=0.18",
                              fc=(0.06, 0.07, 0.09, 0.85), ec=color, lw=0.7))
    ...
    fig.suptitle("Detection overlay — picked lines colored by outcome",
                 fontsize=12, color=th["text"], y=0.99)
...
def build_group_sweep_fig(
    ...
    *, threshold: float = 0.3, theme: str = "light",
    ...
    if not groups:
        ax.text(0.5, 0.5, "No measurements.", ha="center", va="center",
                transform=ax.transAxes, color=th["text_muted"])
        ax.set_axis_off()
        return fig
    ...
    for i, (ch, meas) in enumerate(channel_measurements.items()):
        ...
        ax.bar(x + offset, means, bar_w, label=ch,
               color=_channel_color(ch), edgecolor="white", linewidth=0.8)

    ax.axhline(threshold, color=th["text_muted"], linestyle="--", linewidth=0.9, alpha=0.8)
    ax.text(ax.get_xlim()[1], threshold, f"  t={int(threshold*100)}%",
            va="center", ha="left", color=th["text_muted"], fontsize=9)
    ax.set_xticks(x)
    ...
    ax.set_title("Group sweep — mean contrast per group, per channel",
                 fontsize=12, loc="left", color=th["text"], pad=12)
    ax.legend(loc="upper right", fontsize=9, labelcolor=th["text"],
              facecolor=th["ax_face"], edgecolor=th["grid"], framealpha=0.9,
    ...
    *, theme: str = "light",
    ...
    for i, spec in enumerate(specs):
        ...
        for ch, ms in channel_measurements.items():
            ...
            ax.plot(freqs, mag, color=_channel_color(ch), linewidth=1.1, alpha=0.9,
                    label=ch)
        if first_m is not None and first_m.f_expected_cy_per_sample > 0:
            ax.axvline(first_m.f_expected_cy_per_sample, color=th["text_muted"],
                       linestyle="--", linewidth=0.8, alpha=0.8)
        ax.set_xlim(0, 0.5)
        ax.set_ylim(0, 1.05)
        ax.set_title(f"G{spec.group}E{spec.element}{spec.direction}",
                     fontsize=10, loc="left", color=th["text"])
        ax.set_xlabel("freq (cy/sample)", fontsize=9)
        ax.set_ylabel("|FFT| (norm)", fontsize=9)

    fig.suptitle("FFT spectra — per-line magnitude, bar-fundamental marked",
                 fontsize=12, color=th["text"], y=0.995)
    fig.tight_layout(rect=(0, 0, 1, 0.96))
    return fig
...
class AnalyzeResult:
    ...
    summary_channel: str
    per_channel_detection_limit: Dict[str, Optional[float]]
...
def build_all_usaf_figures(
    ...
    *, threshold: float = 0.3,
    ...
    for ch, img in channel_images.items():
        ...
        for spec in specs:
            try:
                ms.append(measure_line(img, spec, swath_width=float(swath_width),
                                       method=method))
            except ValueError:
    ...
    figs = {
        "mtf_curves": build_mtf_curves_fig(
        ...
        "detection_overlay": build_detection_overlay_fig(
            channel_measurements, channel_images, specs,
            threshold=threshold, theme=theme),
        "group_sweep": build_group_sweep_fig(
            channel_measurements, threshold=threshold, theme=theme),
        "fft_spectra": build_fft_spectra_fig(
            channel_measurements, specs, theme=theme),
    }

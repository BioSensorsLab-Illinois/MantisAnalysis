...
def load_recording(path: str | Path, frame_slice: slice | None = None) -> Recording:
    ...
    with h5py.File(p, "r") as f:
        ...
        it = (
            np.asarray(cam["integration-time"][sl])
            if "integration-time" in cam
            else np.zeros(frames.shape[0])
        )
        ite = (
            np.asarray(cam["integration-time-expected"][sl])
            if "integration-time-expected" in cam
            else it.copy()
        )
        attrs = {}
...
def extract_channel(half: np.ndarray, channel: str, origin: tuple[int, int] = ORIGIN) -> np.ndarray:
...
def extract_rgb_nir(half: np.ndarray, origin: tuple[int, int] = ORIGIN) -> dict[str, np.ndarray]:
...
def split_and_extract(
    frame: np.ndarray, origin: tuple[int, int] = ORIGIN

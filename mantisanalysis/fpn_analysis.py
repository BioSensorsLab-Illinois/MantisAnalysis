...
def _bilateral_simple(image: np.ndarray, sigma_s: float = 2.0, sigma_r: float = 0.05) -> np.ndarray:
...
def _hot_pixel_correct(image: np.ndarray, thr_sigmas: float = 6.0) -> np.ndarray:
...
def percentile_mask(image: np.ndarray, lo_pct: float, hi_pct: float) -> np.ndarray:
...
def _fit_drift_plane(image: np.ndarray, mask: np.ndarray, order: str) -> np.ndarray:
    ...
    *pattern noise* without confounding illumination drift.
    ...
    if order == "bilinear":
        ...
        # 1, x, y, x², y², x*y
        X = np.stack([np.ones_like(xn), xn, yn, xn * xn, yn * yn, xn * yn], axis=-1)
    else:
        return np.zeros_like(image, dtype=np.float64)
...
def _autocorr_from_fpn_map(fpn_map: np.ndarray) -> np.ndarray:
    ...
    ac.flat[ac.size // 2 + ac.shape[1] // 2] if ac.ndim == 2 else ac.max()
...
def _find_hot_pixels(
    image: np.ndarray, mask_kept: np.ndarray, hot_sigma: float
) -> tuple[
    np.ndarray, int, int, list[tuple[int, int, float, float]], list[tuple[int, int, float, float]]
]:
    ...
    if hot_count + cold_count:
        ...
        if cand_hot.size:
            ...
            for k in order:
                ...
                top_hot.append((int(yy), int(xx), float(flat_img[k]), float(flat_z[k])))
        cand_cold = np.where(flat_mask & (flat_z < -hot_sigma))[0]
        if cand_cold.size:
            order = cand_cold[np.argsort(flat_z[cand_cold])][:50]
            for k in order:
                yy, xx = divmod(int(k), W)
                top_cold.append((int(yy), int(xx), float(flat_img[k]), float(flat_z[k])))
...
def compute_fpn(
    ...
    *,
    ...
    settings: FPNSettings | None = None,
) -> FPNResult:
    ...
    if n_kept < 2:
        raise ValueError(
            "After outlier exclusion fewer than 2 pixels remain — loosen the percentile cuts."
        )
    ...
    return FPNResult(
        ...
        dsnu_dn=dsnu_dn,
        prnu_pct=prnu_pct,
        row_noise_dn=row_noise_dn,
        col_noise_dn=col_noise_dn,
        ...
        fpn_map=fpn_map,
        psd_log=psd_log,
        psd_extent=psd_extent,
        autocorr_2d=autocorr_2d,
        autocorr_extent=autocorr_extent,
        row_psd=row_psd,
        col_psd=col_psd,
        row_freq=row_freq,
        col_freq=col_freq,
        row_peak_freq=row_peak_freq,
        col_peak_freq=col_peak_freq,
        row_peak_amp=row_peak_amp,
        col_peak_amp=col_peak_amp,
        hot_pixel_mask=hot_mask,
        hot_pixel_count=hot_count,
        cold_pixel_count=cold_count,
        top_hot=top_hot,
        top_cold=top_cold,
        drift_plane=drift_plane,
        drift_order=drift_order,
        hist_bin_edges=hist_bin_edges_arr,
        hist_counts=hist_counts_arr,
        settings=settings,
    )


def compute_fpn_multi(
    image: np.ndarray,
    *,
    name: str = "",
...
def compute_prnu_stability(
    ...
    *,
    ...
    for f in fracs:
        ...
        try:
            r = compute_fpn(image, name="", roi=(ya, xa, yb, xb), settings=settings)
            out.append(
                {
                    "frac": float(f),
                    "size_h": int(yb - ya),

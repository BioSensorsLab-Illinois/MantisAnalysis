...
const ACCENTS = {
  blue: {
    light: '#1560d9',
    dark: '#4a9eff',
    soft: { light: '#e4efff', dark: '#1a2c47' },
    hover: { light: '#0c4db0', dark: '#3b8ae8' },
  },
  violet: {
    light: '#7c3aed',
    dark: '#a78bfa',
    soft: { light: '#f1eafc', dark: '#2a1f47' },
    hover: { light: '#6027c2', dark: '#916ef7' },
  },
  teal: {
    light: '#0891b2',
    dark: '#2dd4bf',
    soft: { light: '#e0f4f9', dark: '#15323a' },
    hover: { light: '#066b84', dark: '#14b8a6' },
  },
  amber: {
    light: '#c2410c',
    dark: '#fb923c',
    soft: { light: '#fdefe4', dark: '#3c1f0f' },
    hover: { light: '#9a330a', dark: '#ea7b28' },
  },
};
...
const FILE_FILTERS = {
  ...
  images: {
    label: 'All images (PNG/TIFF/JPEG)',
    accept: 'image/png,image/tiff,image/jpeg,.png,.tif,.tiff,.jpg,.jpeg,.bmp',
  },
...
const App = () => {
    ...
    (async () => {
      try {
        ...
        if (list && list.length) {
          ...
          if (alive) {
            setSource(s);
            say('Loaded synthetic sample');
          }
        }
      } catch (err) {
        if (alive) {
          setServerOk(false);
          say(`Server offline: ${err.message}`, 'danger');
        }
      }
    })();
    return () => {
  ...
  useEffectApp(() => {
    if (
      ...
      if (cachedEpoch !== ispEpoch) {
        setAnalysis(null);
        say(
          'ISP reconfigured — analysis cache cleared; re-run Analyze for fresh results.',
          'warning'
        );
      }
    }
  ...
  const onFileChosen = async (e) => {
    ...
    try {
      ...
      setSource(s);
      say(
        `Loaded ${s.name} · ${s.channels.length} channels · ${s.shape[1]}×${s.shape[0]}`,
        'success'
      );
    } catch (err) {
      say(`Upload failed: ${err.message}`, 'danger');
    }
  };
  ...
  const loadSample = async () => {
    try {
      ...
      say(`Sample failed: ${err.message}`, 'danger');
    }
  };
    ...
    <ThemeCtx.Provider value={t}>
      <SourceCtx.Provider value={source}>
        <FileFilterCtx.Provider
          value={{ filters: FILE_FILTERS, current: fileFilter, set: setFileFilter }}
        >
          <div
            data-screen-label={`${mode.toUpperCase()} mode`}
            style={{
              height: '100vh',
              width: '100vw',
              display: 'flex',
          ...
          >
            <input
              ref={fileInputRef}
              type="file"
              accept={FILE_FILTERS[fileFilter]?.accept || ''}
              style={{ display: 'none' }}
            ...
            {showPalette && (
              <CommandPalette actions={actions} onClose={() => setShowPalette(false)} />
            )}
            {showISP && (
              <ISPSettingsWindow
    ...
    >
      <div style={{ maxWidth: 520, textAlign: 'center' }}>
        <div style={{ fontSize: 18, fontWeight: 600, color: t.danger, marginBottom: 8 }}>
          MantisAnalysis server unavailable
        </div>
        <div
          style={{
            fontSize: 12.5,
        ...
        >
          ...
          Then reload. If you opened the HTML directly without a server, MantisAnalysis
          auto-launches one when you run the command above.
        </div>
      </div>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      >
        ...
        <path
          d="M16 16 L28 10 A13.4 13.4 0 0 1 28 22 Z"
          fill={`url(#mantis-spec-${s})`}
          opacity="0.9"
        />
        ...
        />
        <path
          d="M16 3 L16 9 M16 23 L16 29 M3 16 L9 16 M23 16 L29 16"
          stroke="#ffffff"
          strokeWidth="1.6"
          strokeLinecap="round"
        />
    ...
    >
      ...
      {source && (
        ...
        >
          ...
          <span style={{ textOverflow: 'ellipsis', overflow: 'hidden', maxWidth: 240 }}>
            {source.name}
          </span>
          <span style={{ color: t.textFaint }}>
            · {source.channels.length}ch · {source.shape[1]}×{source.shape[0]}
          </span>
          {source.has_dark && (
            <span
              style={{
      ...
      {serverOk === false && (
        <span
          style={{ fontSize: 10.5, color: t.danger, fontFamily: 'ui-monospace,Menlo,monospace' }}
        >
          server offline
        </span>
      )}
      ...
      >
        {[
          ...
          <button
            key={v}
            onClick={() => setThemeName(v)}
            title={`${v[0].toUpperCase() + v.slice(1)} theme`}
            style={{
              width: 26,
              height: 22,
              display: 'flex',
              alignItems: 'center',
              justifyContent: 'center',
              background: themeName === v ? t.panel : 'transparent',
              color: themeName === v ? t.text : t.textMuted,
              border: 'none',
              borderRadius: 4,
              cursor: 'pointer',
              boxShadow: themeName === v ? t.shadow : 'none',
            }}
          >
...
const StatusBar = ({ mode, status, source, onAbout, onPalette }) => {
  ...
  return (
    ...
    >
      ...
      <span>
        {status.count} item{status.count !== 1 ? 's' : ''}
      </span>
      {source && (
        <>
          <span style={{ color: t.textFaint }}>·</span>
          <span title={source.source_id}>src {source.source_id.slice(0, 6)}</span>
        </>
      )}
      <div style={{ flex: 1 }} />
      <button onClick={onPalette} style={chip} title="Command palette (⌘K)">
        <Icon name="search" size={10} /> ⌘K
      </button>
      <span style={{ color: t.textFaint }}>
        v{BRAND.version} · build {BRAND.build}
      </span>
      <button onClick={onAbout} style={chip} title="About · credits">
        <Icon name="info" size={10} /> © {BRAND.year} {BRAND.author}
      </button>
    </div>
  );
};
...
const HelpOverlay = ({ mode, onClose }) => {
  const shortcuts = {
    ...
    usaf: [
      ...
      ['Right-click', 'Delete nearest line'],
    ],
    fpn: [
      ['Drag', 'Define ROI'],
      ['Click outside ROI', 'Clear ROI'],
    ],
    dof: [
      ...
      <div style={{ display: 'grid', gap: 6 }}>
        {items.map(([k, v]) => (
          <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <Kbd>{k}</Kbd>
            <span style={{ fontSize: 11.5, color: t.textMuted }}>{v}</span>
          </div>
        ))}
      </div>
    </div>
...
const AboutOverlay = ({ onClose }) => {
  ...
  return (
    <Modal onClose={onClose} width={520} padding={0}>
      <div
        style={{
          background: `linear-gradient(135deg, #0e9f6e 0%, #10b981 45%, #064e3b 100%)`,
      ...
      >
        ...
        <div style={{ fontSize: 12, opacity: 0.9, lineHeight: 1.55, maxWidth: 380 }}>
          A bench for multi-channel sensor characterization — USAF resolution, fixed-pattern noise,
          and depth-of-field analysis in one place. Python backend (NumPy · SciPy · matplotlib) ·
          React frontend.
        </div>
      </div>
      <div style={{ padding: 22 }}>
        ...
        <AboutSection label="Build">
          ...
          >
            <div>
              version <span style={{ color: t.text }}>{BRAND.version}</span>
            </div>
            <div>
              build <span style={{ color: t.text }}>{BRAND.build}</span>
            </div>
            <div>
              stack{' '}
              <span style={{ color: t.text }}>React 18 · FastAPI · NumPy / SciPy · matplotlib</span>
            </div>
          </div>
        </AboutSection>
        <div
        ...
        >
          © {BRAND.year} {BRAND.author} · {BRAND.authorAffiliation}. Developed for {BRAND.customer}.
          MIT licensed.
        </div>
      </div>
    </Modal>
...
const CommandPalette = ({ actions, onClose }) => {
  ...
  const filtered = useMemoApp(() => {
    ...
    return actions.filter(
      (a) => a.label.toLowerCase().includes(ql) || a.id.toLowerCase().includes(ql)
    );
  }, [q, actions]);
  useEffectApp(() => {
    setSel(0);
  ...
  return (
    <Modal onClose={onClose} width={560} padding={0}>
      ...
      <div style={{ maxHeight: 400, overflowY: 'auto', padding: 6 }}>
        {filtered.length === 0 && (
          <div style={{ padding: 28, textAlign: 'center', color: t.textFaint, fontSize: 12 }}>
            No commands match “{q}”
          </div>
        )}
        {filtered.map((a, i) => (

...
const genId = (prefix) =>
  prefix + Date.now().toString(36) + '_' + Math.floor(Math.random() * 1e4).toString(36);
...
const DoFMode = ({ onRunAnalysis, onStatusChange, say, onSwitchSource, onOpenFile }) => {
  ...
  // ---- Image sizing ------------------------------------------------------
  const [imgDims, setImgDims] = useStateD({
  ...
  const activeHRef = useMemoD(() => {
    ...
    return (activeRefIdH && refsH.find((r) => r.id === activeRefIdH)) || refsH[refsH.length - 1];
  }, [refsH, activeRefIdH]);
  const activeVRef = useMemoD(() => {
    if (!refsV.length) return null;
    return (activeRefIdV && refsV.find((r) => r.id === activeRefIdV)) || refsV[refsV.length - 1];
  }, [refsV, activeRefIdV]);
  ...
  useEffectD(() => {
    ...
    if (!dPoints.length && !dLines.length) {
      setFocus(null);
      return;
    }
    let alive = true;
    setComputing(true);
    apiFetch('/api/dof/compute', {
      method: 'POST',
      body: {
  ...
  useEffectD(() => {
    ...
    if (!firstSelectedLine) {
      setStabCurve(null);
      setStabState('no-line');
      setStabError(null);
      return;
    }
    let alive = true;
    setStabState('loading');
    setStabError(null);
    apiFetch('/api/dof/stability', {
      method: 'POST',
      body: {
        source_id: source.source_id,
        channel: activeChannel,
        p0: firstSelectedLine.p0,
        p1: firstSelectedLine.p1,
        metric,
        threshold,
    ...
    source?.source_id,
    activeChannel,
    firstSelectedLine?.id,
    firstSelectedLine?.p0,
    firstSelectedLine?.p1,
    metric,
    threshold,
    dIspKey,
  ]);
  ...
  const toImg = useCallbackD(
    (ev) => {
      ...
      const fx = px / rw + 0.5,
        fy = py / rh + 0.5;
      return [
        Math.max(0, Math.min(imgDims.w, Math.round(fx * imgDims.w))),
  ...
  const deleteNearest = (pt) => {
    const allPts = points.map((p, i) => ({
      kind: 'pt',
      i,
      id: p.id,
      d: Math.hypot(p.x - pt[0], p.y - pt[1]),
    }));
    const allLn = lines.map((l, i) => ({
      kind: 'ln',
      i,
      id: l.id,
      d: distSegment(pt, l.p0, l.p1),
    }));
  ...
  const importConfig = async (file) => {
    ...
    try {
      ...
      if (cfgPath && cfgPath !== source?.path) {
        try {
          const s = await apiFetch('/api/sources/load-path', {
            method: 'POST',
            body: { path: cfgPath },
          });
          onSwitchSource?.(s);
      ...
      if (Array.isArray(p.analysisChannels))
        setAnalysisChannels(p.analysisChannels.filter((c) => available.includes(c)));
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Card title="Source" icon="open" pinned>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginTop: 8 }}>
            <Button
              ...
              title="Save probes / refs / settings to JSON"
              fullWidth
            >
              Save cfg
            </Button>
            <Button
              ...
              title="Load a previously-saved DoF JSON"
              fullWidth
            >
              Load cfg
            </Button>
        ...
        <Card title={`Analysis channels · ${analysisChannels.length}`} icon="grid">
          ...
          <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
            <Button size="xs" onClick={() => setAnalysisChannels(available)}>
              All
            </Button>
            <Button size="xs" onClick={() => setAnalysisChannels([])}>
              None
            </Button>
            {available.some((c) => c.startsWith('HG-')) && (
              <>
        ...
        <Card title="Focus metric" icon="sliders" pinned>
          ...
          <Slider
            label="Half-window (px)"
            min={8}
            max={96}
            step={1}
            value={halfWin}
            onChange={(v) => setHalfWin(Math.round(v))}
          ...
          <div style={{ fontSize: 10, color: t.textFaint, marginTop: 6, lineHeight: 1.4 }}>
            Larger window = smoother but slower; smaller = faster but noisier. Threshold is the
            focus_norm cut for the DoF band edges.
          </div>
        </Card>
        ...
        <Card title="Research extras" icon="sparkles" pinned>
          <Checkbox
            checked={allMetrics}
            onChange={setAllMetrics}
            label="All 4 metrics"
            hint="Run laplacian / brenner / tenengrad / fft_hf in parallel — cross-check that DoF is metric-independent."
          />
          <Checkbox
            checked={bootstrap}
            onChange={setBootstrap}
            label="Bootstrap 95% CI"
            hint="Percentile bootstrap on peak + DoF width. ~+50 ms per line."
          />
          <Checkbox
            checked={tiltPlane}
            onChange={setTiltPlane}
            label="Tilt-plane fit (needs ≥3 points)"
            hint="Bilinear plane over (x, y, focus) — detects sensor tilt / field curvature."
          />
          <Slider
            label="Target tilt (°)"
            min={0}
      ...
      <DoFCanvas
        ...
        selPoints={selPoints}
        selLines={selLines}
        activeRefIdH={activeHRef?.id}
        activeRefIdV={activeVRef?.id}
        drawing={drawing}
        cursor={cursor}
        rotation={rotation}
        flipH={flipH}
      ...
      >
        ...
        <CalibrationCard
          calibrated={calibrated}
          pxPerMicronH={pxPerMicronH}
          pxPerMicronV={pxPerMicronV}
          pxPerMicronMean={pxPerMicronMean}
          anisotropic={anisotropic}
          refs={refs}
          setRefs={setRefs}
          activeHRef={activeHRef}
          activeVRef={activeVRef}
          setActiveRefIdH={setActiveRefIdH}
          setActiveRefIdV={setActiveRefIdV}
        ...
        <PointsTable
          ...
          onRename={(id, label) =>
            setPoints((p) => p.map((x) => (x.id === id ? { ...x, label } : x)))
          }
        ...
        <LinesTable
          ...
          onRename={(id, label) =>
            setLines((l) => l.map((x) => (x.id === id ? { ...x, label } : x)))
          }
      ...
      <div style={{ flex: 1, position: 'relative', background: t.canvasBg, overflow: 'hidden' }}>
          ...
          >
            {(() => {
              ...
              refs.forEach((r, _i) => {
                ...
                const col = active ? (r.axis === 'h' ? '#ffd54f' : '#2dd4bf') : '#7e57ff';
                  ...
                  <g key={p.id}>
                    ...
                    <g transform={`translate(${sx + 10}, ${sy - 8})`}>
                      <rect
                        x={0}
                        y={-13}
                        width={Math.max(70, 10 + (p.label || '').length * 6)}
                        height={14}
                        rx={3}
              ...
              lines.forEach((l, i) => {
                ...
                if (lr && lr.dof_low_px != null && lr.dof_high_px != null && L_img > 0) {
                  ...
                  dofLo = imgToScreen(
                    l.p0[0] + tLo * (l.p1[0] - l.p0[0]),
                    l.p0[1] + tLo * (l.p1[1] - l.p0[1])
                  );
                  dofHi = imgToScreen(
                    l.p0[0] + tHi * (l.p1[0] - l.p0[0]),
                  ...
                  <g key={l.id}>
                    ...
                    <g transform={`translate(${mx + 6}, ${my - 8})`}>
                      <rect
                        x={0}
                        y={-13}
                        width={Math.max(140, 10 + (l.label || '').length * 6)}
                        height={14}
                        rx={3}
                        fill="rgba(10,10,10,0.9)"
                        stroke={col}
        ...
        <CanvasToolbar position="top-left">
          <CanvasBtn
            ...
            title="Focus tool (P) — click = point · drag = line"
            label="Focus"
          />
          <CanvasBtn
            icon="minus"
            active={tool === 'ref-h'}
            onClick={() => setTool('ref-h')}
        ...
        {(tool === 'ref-h' || tool === 'ref-v') && (
          ...
          >
            <span style={{ color: tool === 'ref-h' ? '#ffd54f' : '#2dd4bf', fontWeight: 600 }}>
        ...
        >
          ...
          >
            <Kbd tone="dim">click</Kbd> point ·<Kbd tone="dim">drag</Kbd> line ·
            <Kbd tone="dim">H/V</Kbd> ref tool ·<Kbd tone="dim">Esc</Kbd> back to Focus ·
            <Kbd tone="dim">rmb</Kbd> delete ·<Kbd tone="dim">space</Kbd> pan ·
...
const DoFRulerH = ({ t, imgSize, _step, ticks, cursorImg, leftInset }) => (
  ...
  >
    <svg width="100%" height={20} style={{ display: 'block' }}>
      {ticks.map((v) => (
        <g key={v}>
          <line
            ...
            strokeWidth={0.6}
          />
          <text x={`${(v / imgSize) * 100}%`} y={10} textAnchor="middle">
            {v}
          </text>
        </g>
      ))}
      {cursorImg != null && (
        <line
          x1={`${(cursorImg / imgSize) * 100}%`}
          y1={0}
          x2={`${(cursorImg / imgSize) * 100}%`}
          y2={20}
          stroke={t.accent}
          strokeWidth={1}
        />
  ...
  >
    <svg width={26} height="100%" style={{ display: 'block' }}>
      {ticks.map((v) => (
        <g key={v}>
          <line
            ...
            strokeWidth={0.6}
          />
          <text x={14} y={`${(v / imgSize) * 100}%`} textAnchor="end" dy={3}>
            {v}
          </text>
        </g>
      ))}
      {cursorImg != null && (
        <line
          x1={0}
          y1={`${(cursorImg / imgSize) * 100}%`}
          x2={26}
          y2={`${(cursorImg / imgSize) * 100}%`}
          stroke={t.accent}
          strokeWidth={1}
        />
      )}
      ...
      {!calibrated ? (
        ...
        >
          ...
          <span style={{ color: pxPerMicronH ? t.success : t.warn }}>
            ...
            {!pxPerMicronH && (
              <span style={{ color: t.textFaint, marginLeft: 6 }}>(fallback = V)</span>
            )}
          </span>
          ...
          <span style={{ color: pxPerMicronV ? t.success : t.warn }}>
            ...
            {!pxPerMicronV && (
              <span style={{ color: t.textFaint, marginLeft: 6 }}>(fallback = H)</span>
            )}
          </span>
          {anisotropic && (
            <>
        ...
        >
          {refsH.map((r) => (
            ...
            >
              ...
              <button
                onClick={() => setRefs((rr) => rr.filter((x) => x.id !== r.id))}
                style={{
...
const PointsTable = ({
  ...
  return (
    <Card
      title={`Points · ${points.length}${selectedIds.size ? ` (${selectedIds.size} sel)` : ''}`}
      icon="pin"
    >
      {points.length === 0 ? (
          ...
          <div style={{ maxHeight: 180, overflowY: 'auto' }}>
            {points.map((p, i) => {
              ...
              return (
                <div
                  key={p.id}
                  onClick={(e) => onToggleSel(p.id, e.nativeEvent)}
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '18px 1fr 44px 44px 40px 18px',
                    gap: 0,
                ...
                >
                  <span
                    style={{ width: 8, height: 8, borderRadius: '50%', background: pointColor(i) }}
                  />
                  {editing === p.id ? (
                    <input
                      autoFocus
                      ...
                      onBlur={(e) => {
                        onRename(p.id, e.target.value.trim() || `p${i + 1}`);
                        setEditing(null);
                      }}
                      style={{
                        width: '100%',
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
        ...
        <Button size="xs" disabled={points.length === 0} onClick={onClearAll}>
          Clear all
        </Button>
      </div>
    </Card>
...
const LinesTable = ({
  ...
  return (
    <Card
      title={`Lines · ${lines.length}${selectedIds.size ? ` (${selectedIds.size} sel)` : ''}`}
      icon="grid"
    >
      {lines.length === 0 ? (
        <div style={{ fontSize: 11, color: t.textFaint, padding: '8px 2px', textAlign: 'center' }}>
          ...
          <div style={{ maxHeight: 200, overflowY: 'auto' }}>
            {lines.map((l, i) => {
              ...
              return (
                <div
                  key={l.id}
                  onClick={(e) => onToggleSel(l.id, e.nativeEvent)}
                  style={{
                    display: 'grid',
                    gridTemplateColumns: '18px 1fr 56px 56px 50px 18px',
                    gap: 0,
                ...
                >
                  <span
                    style={{ width: 8, height: 8, borderRadius: 2, background: lineColor(i) }}
                  />
                  {editing === l.id ? (
                    <input
                      autoFocus
                      ...
                      onBlur={(e) => {
                        onRename(l.id, e.target.value.trim() || `L${i + 1}`);
                        setEditing(null);
                      }}
                      style={{
                        width: '100%',
                  ...
                  <span style={{ color: t.textMuted }}>
                    {peakPx != null ? fmt(peakPx, 1) : '—'}
                  </span>
                  <span style={{ color: t.textMuted }}>
                    {dofWidth != null ? fmt(dofWidth, 1) : '—'}
                  </span>
                  <span
                    style={{
                      color:
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
        ...
        <Button size="xs" disabled={lines.length === 0} onClick={onClearAll}>
          Clear all
        </Button>
        <div style={{ flex: 1 }} />
        <Button
          size="xs"
          icon="export"
          disabled={!lines.length && !focus?.points?.length}
          onClick={onCSV}
        >
          CSV
        </Button>
...
const DoFLinePreview = ({ line, focus, fmt, _calibrated }) => {
  ...
  // matching p0/p1.
  const lineResult = (focus?.lines || []).find(
    (lr) =>
      lr.p0[0] === line.p0[0] &&
      lr.p0[1] === line.p0[1] &&
      lr.p1[0] === line.p1[0] &&
          ...
          >
            ...
            {lineResult.dof_width_ci95_px && (
              <span style={{ color: t.textFaint }}>
                {'  CI95: ['}
                {fmt(lineResult.dof_width_ci95_px[0], 2)}, {fmt(lineResult.dof_width_ci95_px[1], 2)}
                {']'}
              </span>
            )}
            <br />
            {lineResult.gaussian?.converged && (
              <>
                Gaussian σ={fmt(lineResult.gaussian.sigma, 2)} · FWHM=
                {fmt(lineResult.gaussian.fwhm, 2)} · R²={lineResult.gaussian.r_squared.toFixed(3)}
              </>
            )}
          </div>
        </>
    ...
    <Card title="DoF stability curve" icon="settings" actions={actions}>
      {!enabled ? (
        <div style={{ fontSize: 10.5, color: t.textFaint, lineHeight: 1.5 }}>
          <div style={{ fontWeight: 500, color: t.textMuted, marginBottom: 4 }}>
            What this checks
          </div>
          {helpText}
    ...
    <div>
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block' }}>
        ...
        {curve.map((p, i) => (
          <circle
            key={i}
            cx={xOf(p.half_window)}
            cy={yOf(p.dof_width_px || 0)}
            r={3}
            fill={t.accent}
            stroke="#fff"
            strokeWidth={0.8}
      ...
      >
        half-win → DoF:{' '}
        {curve.map((p) => `${p.half_window}=${fmt(p.dof_width_px || 0, 1)}`).join(' · ')}
      </div>
    </div>
  );
};
...
const DoFTiltPlaneCard = ({ tilt, n_points, enabled, computing, error, activeChannel }) => {
  ...
  return (
    <Card
      title="Tilt plane / field curvature"
      icon="pin"
      actions={
        <Tip title={helpText}>
          <span
    ...
    >
      {!enabled ? (
        <div style={{ fontSize: 10.5, color: t.textFaint, lineHeight: 1.5 }}>
          ...
          {helpText}
          <div style={{ marginTop: 6, color: t.textFaint }}>
            Enable <b>Tilt-plane fit</b> in the <b>Research extras</b> card on the left, then drop
            at least 3 focus points in regions with different sharpness.
          </div>
        </div>
      ) : error ? (
        <div style={{ fontSize: 11, color: t.danger, padding: '10px 4px', lineHeight: 1.4 }}>
        ...
        >
          ...
          <span>
            Need <b>≥3 points</b> to fit a plane — currently {n_points}. Switch to the Focus tool
            and click more places on the canvas.
          </span>
        </div>
      ) : computing && !tilt ? (
        <div
        ...
        <div style={{ fontSize: 10.5, color: t.textFaint, lineHeight: 1.5 }}>
          Drop ≥3 focus points on different features to fit a plane. A near-zero slope = flat sensor
          / well-focused; a meaningful slope = sensor tilt or field curvature.
        </div>
      ) : (
        <StatBlock
          emphasis="slope"
          items={[
            ...
            { label: 'coef c (∂focus/∂y)', value: tilt.c.toExponential(2) },
            {
              ...
              color:
                tilt.slope_mag_per_px < 1e-4
                  ? t.success
                  : tilt.slope_mag_per_px < 1e-3
                    ? t.warn
                    : t.danger,
            },

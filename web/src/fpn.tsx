...
const genROIId = () =>
  'r' + Date.now().toString(36) + '_' + Math.floor(Math.random() * 1e4).toString(36);
...
const FPNMode = ({ onRunAnalysis, onStatusChange, say, onSwitchSource, onOpenFile }) => {
  ...
  // ---- Sizing -------------------------------------------------------------
  const [imgDims, setImgDims] = useStateF({
  ...
  const dSettings = useDebounced(
    { medianSize, gaussSigma, hotPixThr, bilateral, loPct, hiPct, driftOrder, hotSigma },
    220
  );

  // ---- Live per-ROI measurement (small-summary /api/fpn/compute) ---------
  const measureOne = useCallbackF(
    async (roi) => {
      ...
      try {
        return await apiFetch('/api/fpn/compute', { method: 'POST', body });
      } catch (err) {
  ...
  useEffectF(() => {
    if (!stabEnabled || !source || !selectedLine) {
      setStabCurve(null);
      return;
    }
    let alive = true;
  ...
  const addRoi = async (x0, y0, x1, y1) => {
    ...
    let roi;
    setRois((prev) => {
      roi = { id, x0: xa, y0: ya, x1: xb, y1: yb, label: `ROI-${prev.length + 1}`, pending: true };
      return [...prev, roi];
    });
    ...
    setRois((prev) =>
      prev.map((r) =>
        r.id === id
          ? { ...r, m: m && !m.__error ? m : null, error: m?.__error || null, pending: false }
          : r
      )
    );
  };
      ...
      prev.map((r) =>
        r.id === id
          ? {
              ...r,
              ...patch,
              pending: patch.x0 != null || patch.y0 != null || patch.x1 != null || patch.y1 != null,
            }
          : r
      )
    );
  ...
  useEffectF(() => {
    const down = (e) => {
      ...
      if (e.key === 'Delete' || e.key === 'Backspace') {
        if (selectedIds.size) deleteSelected();
      }
  ...
  // ---- Status bar --------------------------------------------------------
  useEffectF(() => {
    const worst = rois.length
      ? rois.reduce(
          (acc, r) =>
            r.m && r.m.prnu_pct != null && r.m.prnu_pct > (acc?.prnu_pct ?? -1) ? r.m : acc,
          null
        )
      : null;
    ...
    try {
      ...
      if (cfgPath && cfgPath !== source?.path) {
        try {
          ...
          say?.(
            `Could not auto-load source from ${cfgPath} (${err.detail || err.message}); using current source`,
            'warn'
          );
        }
      ...
      if (cfg.dark?.path && activeSrc) {
        ...
        say?.(
          `Config references dark "${cfg.dark.name}" but no path is stored (browser upload). Re-attach via the Dark frame card.`,
          'warn'
        );
      }
      const p = cfg.picker || {};
      ...
      say?.(
        `Loaded ${cfg.rois?.length ?? 0} ROI${cfg.rois?.length === 1 ? '' : 's'} from ${file.name}`,
        'success'
      );
    } catch (err) {
  ...
  const exportCSVTable = () => {
    ...
    if (!rows.length) {
      say?.('Nothing to export — no measured ROIs.', 'warn');
      return;
    }
    ...
    >
      ...
      >
        ...
        <Card title="Source" icon="open" pinned>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginTop: 8 }}>
            <Button
              ...
              title="Save picker / settings / ROIs to JSON"
              fullWidth
            >
              Save cfg
            </Button>
            <Button
              ...
              title="Load a previously-saved JSON; ROIs will be re-measured"
              fullWidth
            >
              Load cfg
            </Button>
          ...
          >
            ...
            >
              ...
              <Button
                ...
                onClick={onClearDark}
                title="Detach the dark frame; FPN stats return to raw DN"
                fullWidth
              >
                Clear
              </Button>
        ...
        <Card title="Drift removal" icon="sliders" pinned>
          ...
          <div style={{ fontSize: 10, color: t.textFaint, marginTop: 6, lineHeight: 1.4 }}>
            <b>Linear</b> catches simple tilt · <b>Quadratic</b> catches vignette. With drift off,
            residual tilt counts as DSNU.
          </div>
        </Card>
        ...
        <Card title="Outlier / hot-pixel sensitivity" icon="sparkles">
          ...
          <Slider
            ...
            step={0.1}
            value={hotSigma}
            onChange={setHotSigma}
            format={(v) => v.toFixed(1)}
          />
          <div style={{ fontSize: 10, color: t.textFaint, marginTop: 6, lineHeight: 1.4 }}>
            Percentile cuts exclude bright/dark outliers from the σ. <b>|z| hot-pixel</b> is the
            threshold for flagging individual hot/cold pixels on the analysis map.
          </div>
        </Card>
        ...
        <Card title="ISP / smoothing" icon="isp">
          ...
          <Slider
            ...
            step={0.05}
            value={gaussSigma}
            onChange={setGaussSigma}
          />
          <Slider
            label="Hot-pix repl. (>σ)"
            min={0}
            max={20}
            step={0.5}
            value={hotPixThr}
            onChange={setHotPixThr}
            format={(v) => v.toFixed(1)}
          />
          <Checkbox
            checked={bilateral}
      ...
      >
        ...
        <ROIsTable
          ...
          onRename={(id, label) => updateRoi(id, { label })}
        ...
        <Card title="Aggregate" icon="pin" pinned>
              ...
              <StatBlock
                ...
                items={[
                  ...
                  { label: 'σ residual (avg)', value: `${fmtDN(resAvg)} DN`, muted: true },
                  {
                    label: 'hot-pix total',
                    value: `${hotSum}`,
                    color: hotSum === 0 ? t.success : t.warn,
                    muted: hotSum === 0,
                  },
                ]}
              />
          ...
          <div style={{ marginTop: 8, fontSize: 10, color: t.textFaint, lineHeight: 1.4 }}>
            Peak PRNU is the worst-case uniformity across all ROIs; hot-pix total sums across ROIs
            at |z| &gt; {hotSigma.toFixed(1)}.
          </div>
        </Card>
      </div>
    </div>
    ...
    <div style={{ background: t.bg, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
      ...
      >
        <div>
          ...
          >
            ...
            {flipV ? ' · V' : ''} · zoom {(zoom * 100).toFixed(0)}% · {rois.length} ROI
          </div>
        </div>
        <div
          style={{
      ...
      <div style={{ flex: 1, position: 'relative', background: t.canvasBg, overflow: 'hidden' }}>
                  ...
                  <g key={r.id}>
                    ...
                    <g transform={`translate(${x + 5}, ${y - 6})`}>
                      <rect
                        x={0}
                        y={-15}
                        width={Math.max(70, (r.label || '').length * 7)}
                        height={15}
                        rx={3}
                    ...
                    {r.m && (
                      <g transform={`translate(${x + w - 4}, ${y + 4})`}>
                        <rect
                          x={-70}
                          y={0}
                          width={70}
        ...
        >
          ...
          >
            <Kbd tone="dim">drag</Kbd> ROI ·<Kbd tone="dim">space</Kbd> pan ·
            <Kbd tone="dim">wheel</Kbd> zoom ·<Kbd tone="dim">rmb</Kbd> delete ·
            <Kbd tone="dim">click</Kbd> select ·<Kbd tone="dim">⌘Z</Kbd> undo
          </div>
        </div>
      </div>
    </div>
          ...
          return (
            <g key={v}>
              <line
                ...
                strokeWidth={0.6}
              />
              <text x={`${frac * 100}%`} y={10} textAnchor="middle">
                {v}
              </text>
            </g>
          );
          ...
          return (
            <g key={v}>
              <line
                ...
                strokeWidth={0.6}
              />
              <text x={14} y={`${frac * 100}%`} textAnchor="end" dy={3}>
                {v}
              </text>
            </g>
          );
...
const ROIsTable = ({
  ...
  return (
    <Card
      title={`ROIs · ${totalROIs}${selectedIds.size ? ` (${selectedIds.size} sel)` : ''}`}
      icon="grid"
      pinned
    >
        ...
        <div style={{ maxHeight: 220, overflowY: 'auto', background: t.panel }}>
          {rois.map((r, _idx) => {
            ...
            return (
              <div
                key={r.id}
                onClick={(e) => toggleSel(r.id, e.nativeEvent)}
                style={{
                  display: 'grid',
                  gridTemplateColumns: cg,
                  gap: 0,
              ...
              >
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 4, overflow: 'hidden' }}>
                  ...
                  {editingId === r.id ? (
                    <input
                      ...
                      onBlur={(e) => {
                        onRename?.(r.id, e.target.value.trim() || `ROI-${roiIdx + 1}`);
                        setEditingId(null);
                      }}
                      style={{
                        width: '100%',
                ...
                <div style={{ color: t.textMuted }}>
                  {r.m ? fmtDN(r.m.residual_pixel_noise_dn) : '—'}
                </div>
      ...
      <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
        ...
        <Tip title="Select all">
          <Button size="xs" onClick={onSelectAll} disabled={totalROIs === 0}>
            Select all
          </Button>
        </Tip>
        <Tip title="Delete all ROIs">
          <Button size="xs" onClick={onClearAll} disabled={totalROIs === 0}>
            Clear all
          </Button>
        </Tip>
        <div style={{ flex: 1 }} />
        <Tip title="Download measured ROIs as CSV">
          <Button size="xs" icon="export" disabled={totalROIs === 0} onClick={onCSV}>
            CSV
          </Button>
        </Tip>
      </div>
...
const FPNLiveStats = ({ roi, _full, multiCount }) => {
  ...
  if (!roi) {
    return (
      <Card title="Selected ROI stats" icon="pin" pinned>
    ...
    <Card title={`${roi.label || 'ROI'} · stats`} icon="pin" pinned>
      ...
      <StatBlock
        ...
        items={[
          ...
          {
            label: 'row peak f',
            value: m.row_peak_freq ? `${m.row_peak_freq.toFixed(4)} cy` : '—',
            muted: true,
          },
          {
            label: 'col peak f',
            value: m.col_peak_freq ? `${m.col_peak_freq.toFixed(4)} cy` : '—',
            muted: true,
          },
          {
            label: 'hot / cold',
            value: `${m.hot_pixel_count} / ${m.cold_pixel_count}`,
            color: m.hot_pixel_count === 0 && m.cold_pixel_count === 0 ? t.success : t.warn,
          },
          {
            label: 'kept px',
            value: `${m.n_kept.toLocaleString()} / ${m.n_total.toLocaleString()}`,
            muted: true,
          },
...
const FPNProfilePreview = ({ roi, full }) => {
  ...
  if (!full) {
    return (
      <Card title="Profile preview" icon="eye" pinned>
        <div style={{ fontSize: 11, color: t.textFaint, padding: '14px 4px', textAlign: 'center' }}>
          Loading profiles…
        </div>
      </Card>
...
const MiniProfile = ({ title, values, color }) => {
  ...
  const lo = Math.min(...valid),
    hi = Math.max(...valid);
  const mid = (lo + hi) / 2,
    range = Math.max(1e-9, hi - lo) * 1.08;
  const N = vs.length;
  const pts = vs
    .map((v, i) => {
    ...
    <Card title="PRNU stability curve" icon="settings" actions={actions}>
      {!roi ? (
        ...
        <div style={{ fontSize: 10.5, color: t.textFaint, lineHeight: 1.4 }}>
          Shrinks the ROI concentrically and plots PRNU vs ROI size. A flat curve = statistics have
          stabilised. Enable when you want to check whether the ROI is large enough for a
          trustworthy PRNU number.
        </div>
      ) : !curve ? (
        <div style={{ fontSize: 11, color: t.textFaint, padding: '10px 4px', textAlign: 'center' }}>
          computing…

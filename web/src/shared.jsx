...
const modeGrid = ({ leftOpen = true, rightOpen = true, leftW = 320, rightW = 368, minCanvas = 480 } = {}) => {
...
const PLOT_STYLE_PRESETS = {
  ...
  publication: {
    ...
    titleSize: 13, titleWeight: 600, axisLabelSize: 12, axisLabelWeight: 500,
    tickSize: 10, legendSize: 10, legendWeight: 400,
    lineWidth: 1.2, markerSize: 5, markerStrokeWidth: 0.6,
    gridOpacity: 0.35, axisStrokeWidth: 0.9, gridWidth: 0.4,
    cardBackground: 'white', cardBorder: true, cardBorderRadius: 4,
    gridGap: 10, cardPadding: 8,
    palette: 'channel',
    exportScale: 3, exportBackground: 'white',
  },
  presentation: {
    ...DEFAULT_PLOT_STYLE,
    fontFamily: 'Inter Tight',
    titleSize: 18, titleWeight: 700, axisLabelSize: 15, axisLabelWeight: 600,
    tickSize: 13, legendSize: 14, legendWeight: 600,
    lineWidth: 2.4, markerSize: 10, markerStrokeWidth: 1.2,
    gridOpacity: 0.4, axisStrokeWidth: 1.2,
    cardBackground: 'panel', cardBorderRadius: 10, cardPadding: 14,
    gridGap: 18,
    palette: 'channel',
    exportScale: 2, exportBackground: 'white',
  },
  compact: {
    ...DEFAULT_PLOT_STYLE,
    fontFamily: 'Inter Tight',
    titleSize: 11, titleWeight: 600, axisLabelSize: 10, axisLabelWeight: 400,
    tickSize: 8, legendSize: 9, legendWeight: 400,
    lineWidth: 1.2, markerSize: 4, markerStrokeWidth: 0.5,
    gridOpacity: 0.25, axisStrokeWidth: 0.5, gridWidth: 0.3,
    cardBackground: 'panel', cardBorderRadius: 6, cardPadding: 6,
    gridGap: 8,
    exportScale: 2, exportBackground: 'auto',
  },
};
...
const cardChromeFor = (style, t) => {
  const bg = style.cardBackground === 'white' ? '#ffffff'
...
const usePlotStyleState = () => {
  ...
  const setStyle = (patch) => {
    if (typeof patch === 'function') setStyleFull(prev => ({ ...DEFAULT_PLOT_STYLE, ...prev, ...patch(prev) }));
...
const Icon = ({ name, size = 14, style }) => {
  ...
  const P = {
    usaf: <g><rect x="2" y="2" width="12" height="12" rx="1" /><path d="M5 4v8M7 5v6M9 4v8M11 6v4" /></g>,
    fpn: <g><rect x="2.5" y="2.5" width="11" height="11" rx="1" /><circle cx="5.5" cy="5.5" r="0.6" fill="currentColor" stroke="none"/><circle cx="10.5" cy="6" r="0.6" fill="currentColor" stroke="none"/><circle cx="7" cy="9" r="0.6" fill="currentColor" stroke="none"/><circle cx="11" cy="11" r="0.6" fill="currentColor" stroke="none"/><circle cx="4.5" cy="11" r="0.6" fill="currentColor" stroke="none"/></g>,
    dof: <g><circle cx="8" cy="8" r="5" /><circle cx="8" cy="8" r="2" /><path d="M8 1v1.5M8 13.5V15M1 8h1.5M13.5 8H15" /></g>,
    open: <g><path d="M2 5l2-2h4l1 1h5v8a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V5z"/></g>,
    play: <g><path d="M5 3l8 5-8 5V3z" fill="currentColor"/></g>,
    sun: <g><circle cx="8" cy="8" r="3"/><path d="M8 1.5v1.5M8 13v1.5M1.5 8h1.5M13 8h1.5M3.5 3.5l1 1M11.5 11.5l1 1M3.5 12.5l1-1M11.5 4.5l1-1"/></g>,
    moon: <g><path d="M13 9a5 5 0 0 1-7-6 5 5 0 1 0 7 6z"/></g>,
    close: <g><path d="M4 4l8 8M12 4l-8 8"/></g>,
    chevron: <g><path d="M5 6l3 3 3-3"/></g>,
    rotate: <g><path d="M2 8a6 6 0 1 1 2 4.5"/><path d="M2 12v-3h3"/></g>,
    flip: <g><path d="M8 2v12"/><path d="M5 5l-2 3 2 3M11 5l2 3-2 3"/></g>,
    zoomReset: <g><rect x="2" y="2" width="12" height="12" rx="1"/><path d="M6 6h4v4H6z"/></g>,
    undo: <g><path d="M6 4L2 8l4 4"/><path d="M2 8h8a4 4 0 0 1 4 4"/></g>,
    trash: <g><path d="M3 4h10M6 4V3a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1v1M4.5 4l.5 9a1 1 0 0 0 1 1h4a1 1 0 0 0 1-1l.5-9"/></g>,
    run: <g><path d="M4 3l9 5-9 5V3z" fill="currentColor" stroke="none"/></g>,
    export: <g><path d="M8 2v8M5 6l3-4 3 4M3 11v2a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-2"/></g>,
    help: <g><circle cx="8" cy="8" r="6"/><path d="M6.5 6.5a1.5 1.5 0 1 1 2.2 1.3c-.5.3-.7.7-.7 1.2M8 11.5v.1"/></g>,
    keyboard: <g><rect x="1.5" y="4.5" width="13" height="7" rx="1"/><path d="M4 7h.01M6 7h.01M8 7h.01M10 7h.01M12 7h.01M4 9.5h8"/></g>,
    search: <g><circle cx="7" cy="7" r="4"/><path d="M10 10l3 3"/></g>,
    plus: <g><path d="M8 3v10M3 8h10"/></g>,
    minus: <g><path d="M3 8h10"/></g>,
    check: <g><path d="M3 8l3 3 7-7"/></g>,
    drag: <g><circle cx="6" cy="4" r="0.8" fill="currentColor" stroke="none"/><circle cx="10" cy="4" r="0.8" fill="currentColor" stroke="none"/><circle cx="6" cy="8" r="0.8" fill="currentColor" stroke="none"/><circle cx="10" cy="8" r="0.8" fill="currentColor" stroke="none"/><circle cx="6" cy="12" r="0.8" fill="currentColor" stroke="none"/><circle cx="10" cy="12" r="0.8" fill="currentColor" stroke="none"/></g>,
    sliders: <g><path d="M3 4h7M12 4h1M3 8h2M7 8h6M3 12h8M10 12h3"/><circle cx="11" cy="4" r="1"/><circle cx="6" cy="8" r="1"/><circle cx="9" cy="12" r="1"/></g>,
    ...
    pin: <g><path d="M7 2h2l1 4 2 1v1H4V7l2-1 1-4z"/><path d="M8 9v5"/></g>,
    eye: <g><path d="M1 8s2.5-5 7-5 7 5 7 5-2.5 5-7 5-7-5-7-5z"/><circle cx="8" cy="8" r="2"/></g>,
    ...
    save: <g><path d="M3 2h8l3 3v9a1 1 0 0 1-1 1H3a1 1 0 0 1-1-1V3a1 1 0 0 1 1-1z"/><path d="M5 2v4h6V2M5 10h6v5"/></g>,
    upload: <g><path d="M8 11V3M5 6l3-3 3 3M3 11v2a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-2"/></g>,
    sparkles: <g><path d="M8 2l1.3 3.7L13 7l-3.7 1.3L8 12l-1.3-3.7L3 7l3.7-1.3L8 2z"/><path d="M12.5 11.5l.5 1.5.5-1.5 1.5-.5-1.5-.5-.5-1.5-.5 1.5-1.5.5 1.5.5z"/></g>,
    isp: <g><rect x="2" y="2" width="12" height="12" rx="1"/><path d="M5 8l2-2 2 3 3-4"/><circle cx="5" cy="8" r="0.8" fill="currentColor" stroke="none"/><circle cx="12" cy="5" r="0.8" fill="currentColor" stroke="none"/></g>,
    hand: <g><path d="M6 7V3.5a1.2 1.2 0 0 1 2.4 0V7M8.4 7V2.5a1.2 1.2 0 0 1 2.4 0V7M10.8 7V3a1.2 1.2 0 0 1 2.4 0V9a5 5 0 0 1-5 5H7a3 3 0 0 1-3-3V7a1.2 1.2 0 0 1 2.4 0"/></g>,
    crosshair: <g><circle cx="8" cy="8" r="2"/><path d="M8 1v3M8 12v3M1 8h3M12 8h3"/></g>,
    fit: <g><path d="M3 6V3h3M13 6V3h-3M3 10v3h3M13 10v3h-3"/></g>,
    info: <g><circle cx="8" cy="8" r="6"/><path d="M8 7.5v3.5M8 5.5v.01"/></g>,
    cmd: <g><path d="M5 3a2 2 0 1 0 0 4h6a2 2 0 1 0 0-4 2 2 0 0 0-2 2v6a2 2 0 1 0 2-2H5a2 2 0 1 0 0 2 2 2 0 0 0 2-2V5a2 2 0 0 0-2-2z"/></g>,
    download: <g><path d="M8 2v8M5 7l3 3 3-3M3 11v2a1 1 0 0 0 1 1h8a1 1 0 0 0 1-1v-2"/></g>,
    image: <g><rect x="2" y="3" width="12" height="10" rx="1"/><circle cx="6" cy="7" r="1.2"/><path d="M2 11l3-3 3 3 2-2 3 3"/></g>,
    palette: <g><path d="M8 2a6 6 0 1 0 0 12c1 0 1-1 0-1.5-.7-.4-.4-1.5.5-1.5h2a3 3 0 0 0 3-3c0-3.3-2.7-6-6-6z"/><circle cx="5.5" cy="7" r="0.8" fill="currentColor" stroke="none"/><circle cx="8" cy="5" r="0.8" fill="currentColor" stroke="none"/><circle cx="10.5" cy="7" r="0.8" fill="currentColor" stroke="none"/></g>,
    mantis: <g><path d="M7 13c0-1.5 1-2.5 2-2.5s2 1 2 2.5M8 10.5V9M8 8V6M6 5l2 1 2-1M5.5 4.5l.5 1M10.5 4.5l-.5 1M6.5 3.5l1.5 1 1.5-1"/></g>,
  };
...
// ---------------------------------------------------------------------------
const Card = ({ title, icon, children, defaultOpen = true, pinned = false, actions, dense = false }) => {
  const t = useTheme();
    ...
    <div style={{
      ...
      <button
        ...
        style={{
          ...
          cursor: pinned ? 'default' : 'pointer', color: t.textMuted, textAlign: 'left',
        }}
      >
        {icon && <span style={{ color: t.textFaint, display: 'flex' }}><Icon name={icon} size={13} /></span>}
        <span style={{
        ...
        {actions}
        {!pinned && (
          <span style={{ color: t.textFaint, display: 'flex', transform: open ? 'rotate(0)' : 'rotate(-90deg)', transition: 'transform .15s' }}>
            <Icon name="chevron" size={12} />
          </span>
        )}
      </button>
      {open && (
        <div style={{ padding: dense ? '2px 10px 10px' : '4px 10px 12px' }}>
          {children}
        </div>
      )}
    </div>
...
const Row = ({ label, children, align = 'center' }) => {
  const t = useTheme();
  return (
    <div style={{ display: 'flex', alignItems: align, gap: 10, marginTop: 8, minHeight: 24, minWidth: 0 }}>
      {label && <div style={{ flex: '0 0 78px', fontSize: 11.5, color: t.textMuted, lineHeight: '24px' }}>{label}</div>}
...
// ---------------------------------------------------------------------------
const Slider = ({ label, min, max, step = 0.01, value, onChange, format = v => v.toFixed(2), unit = '' }) => {
  const t = useTheme();
  ...
  return (
    <div style={{ marginTop: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11.5, color: t.textMuted, marginBottom: 4 }}>
        ...
        {editing ? (
          <input
            ...
            onBlur={(e) => { const v = parseFloat(e.target.value); if (!isNaN(v)) onChange(Math.max(min, Math.min(max, v))); setEditing(false); }}
            onKeyDown={(e) => { if (e.key === 'Enter') e.target.blur(); if (e.key === 'Escape') setEditing(false); }}
            style={{
              width: 60, fontSize: 11.5, padding: '1px 4px',
              background: t.inputBg, color: t.text,
              border: `1px solid ${t.accent}`, borderRadius: 3, fontFamily: 'ui-monospace,Menlo,monospace',
            }}
          />
        ) : (
          <span
            ...
            style={{
              ...
              color: t.text, cursor: 'text', padding: '1px 4px', borderRadius: 3,
              background: 'transparent',
            }}
            onMouseEnter={(e) => e.currentTarget.style.background = t.chipBg}
            onMouseLeave={(e) => e.currentTarget.style.background = 'transparent'}
          >{format(value)}{unit}</span>
        )}
      </div>
      <div style={{ position: 'relative', height: 18, display: 'flex', alignItems: 'center' }}>
        <div style={{ position: 'absolute', left: 0, right: 0, height: 4, background: t.chipBg, borderRadius: 2 }} />
        ...
        <input
          ...
          style={{
            ...
            height: 18, position: 'relative', zIndex: 1, margin: 0, cursor: 'pointer',
          }}
          className="rgbnir-slider"
        />
        {dragging && (
          <div style={{
            position: 'absolute', left: `calc(${pct}% - 20px)`, bottom: 22,
            background: t.text, color: t.panel,
            fontFamily: 'ui-monospace,SF Mono,Menlo,monospace', fontSize: 10.5,
            padding: '2px 6px', borderRadius: 3, whiteSpace: 'nowrap',
            pointerEvents: 'none',
          }}>{format(value)}{unit}</div>
        )}
      </div>
    </div>
  );
    ...
    <div style={{ position: 'relative', flex: 1 }}>
      <select
        ...
        style={{
          ...
          fontSize: size === 'sm' ? 11.5 : 12.5, padding: size === 'sm' ? '3px 22px 3px 8px' : '5px 24px 5px 10px',
      ...
      <span style={{ position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', color: t.textFaint, pointerEvents: 'none', display: 'flex' }}>
        <Icon name="chevron" size={12} />
      </span>
    </div>
  );
...
// ---------------------------------------------------------------------------
const Button = ({ variant = 'ghost', icon, iconRight, size = 'sm', children, onClick, disabled, title, fullWidth, active }) => {
  const t = useTheme();
  ...
  return (
    <button
      ...
      style={{
        ...
        padding: pad, fontSize: fs, fontWeight: variant === 'primary' ? 500 : 450,
        background: styles.bg, color: styles.fg,
        border: `1px solid ${styles.border}`, borderRadius: 5,
  ...
  return (
    ...
    >
      <span style={{
        ...
        background: color, boxShadow: band === 'Y' ? 'inset 0 0 0 1px rgba(0,0,0,.2)' : 'none',
        flexShrink: 0,
      }} />
...
const Segmented = ({ value, options, onChange, size = 'sm', fullWidth = false }) => {
  ...
  return (
    <div style={{
      display: fullWidth ? 'flex' : 'inline-flex', background: t.chipBg,
      border: `1px solid ${t.chipBorder}`, borderRadius: 6, padding: 2, gap: 2,
        ...
        return (
          <button
            ...
            style={{
              ...
              fontSize: size === 'xs' ? 10.5 : 11.5, fontWeight: active ? 500 : 400,
              background: active ? t.panel : 'transparent',
              color: active ? t.text : t.textMuted,
              border: 'none', borderRadius: 4, cursor: 'pointer',
              boxShadow: active ? t.shadow : 'none',
              fontFamily: 'inherit', flex: fullWidth ? 1 : '0 0 auto',
  ...
  return (
    <label style={{
      ...
      <div style={{
        ...
        flexShrink: 0, marginTop: 1, color: '#fff',
      }}>
...
const Spinbox = ({ value, min, max, step = 1, onChange, width = 54 }) => {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', border: `1px solid ${t.border}`, borderRadius: 4, overflow: 'hidden', background: t.inputBg }}>
      <button
        onClick={() => onChange(clamp(value - step))}
        style={{ border: 'none', background: 'transparent', color: t.textMuted, padding: '2px 6px', cursor: 'pointer', fontSize: 11 }}
      >−</button>
      <input
        type="number"
        value={value}
        ...
        style={{
          width, border: 'none', background: 'transparent', color: t.text,
          fontSize: 11.5, textAlign: 'center', padding: '3px 0',
...
const StatBlock = ({ items, emphasis }) => {
  ...
  return (
    <div style={{
      ...
      padding: '8px 10px', lineHeight: 1.55,
    }}>
      {items.map((it, i) => (
        <div key={i} style={{
...
const HUD = ({ children, style }) => {
  ...
  return (
    <div style={{
      position: 'absolute', background: 'rgba(20,24,31,0.88)', color: '#fff',
...
const makeUSAFImage = (w = 720, h = 540) => {
  ...
  g.addColorStop(0, 'rgba(40,42,48,0.3)'); g.addColorStop(1, 'rgba(0,0,0,0.5)');
  ...
  ctx.restore();
  // faint chart frame
  ctx.strokeStyle = 'rgba(200,204,212,0.15)'; ctx.lineWidth = 1;
...
const makeFPNImage = (w = 720, h = 540) => {
  ...
  for (let y = 0; y < h; y++) {
    ...
    else { ctx.fillStyle = `rgba(0,0,0,${Math.abs(rb) / 120})`; ctx.fillRect(0, y, w, 1); }
  }
  // column FPN
  for (let x = 0; x < w; x++) {
    const cb = (Math.random() - 0.5) * 4 + Math.cos(x * 0.12) * 1.5;
    if (cb > 0) { ctx.fillStyle = `rgba(255,255,255,${Math.abs(cb) / 140})`; ctx.fillRect(x, 0, 1, h); }
    else { ctx.fillStyle = `rgba(0,0,0,${Math.abs(cb) / 140})`; ctx.fillRect(x, 0, 1, h); }
  }
  // Hot pixels
  ctx.fillStyle = '#fff';
...
const makeDoFImage = (w = 720, h = 540) => {
  ...
  for (let i = 0; i < 180; i++) {
    ...
    ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(60, y); ctx.lineTo(w - 60, y + 10); ctx.stroke();
  }
  ctx.filter = 'none';
  ...
  g.addColorStop(0, 'rgba(30,32,36,0)'); g.addColorStop(1, 'rgba(0,0,0,0.45)');
  ctx.fillStyle = g; ctx.fillRect(0, 0, w, h);
  return c.toDataURL('image/png');
};
...
const Modal = ({ children, onClose, width = 480, label, padding = 20 }) => {
  ...
  return (
    <div onClick={onClose} aria-label={label} style={{
      position: 'fixed', inset: 0, background: 'rgba(8,10,14,0.55)', zIndex: 100,
      display: 'flex', alignItems: 'center', justifyContent: 'center', backdropFilter: 'blur(2px)',
    }}>
      <div onClick={(e) => e.stopPropagation()} style={{
        background: t.panel, border: `1px solid ${t.border}`, borderRadius: 10,
        padding, width, maxWidth: 'calc(100vw - 32px)', maxHeight: '92vh', overflow: 'auto',
...
const Toast = ({ msg, kind = 'info', onDone, duration = 2200 }) => {
  ...
  return (
    <div style={{
      ...
      background: colors.bg, color: colors.fg, padding: '7px 14px', borderRadius: 6,
      fontSize: 12, fontWeight: 500, boxShadow: t.shadowLg, zIndex: 200,
...
const useLocalStorageState = (key, initial) => {
  ...
  const [v, setV] = useState(() => {
    try { const raw = localStorage.getItem(fullKey); if (raw != null) return JSON.parse(raw); } catch {}
...
// ---------------------------------------------------------------------------
const useImageMouse = (canvasRef, zoom = 1, pan = [0, 0], rotation = 0, flipH = false, flipV = false) => {
  ...
  return useCallback((ev) => {
    ...
    const x = Math.max(0, Math.min(IMAGE_DIMS.W, ((ev.clientX - r.left) / r.width) * IMAGE_DIMS.W));
    const y = Math.max(0, Math.min(IMAGE_DIMS.H, ((ev.clientY - r.top) / r.height) * IMAGE_DIMS.H));
...
const exportJSON = (filename, obj) => downloadBlob(filename, 'application/json', JSON.stringify(obj, null, 2));
...
const CanvasColorbar = ({ colormap, vmin, vmax, h = 220, w = 14, side = 'right' }) => {
  ...
  return (
    <div style={{
      position: 'absolute', top: 12, ...pos,
      display: 'flex', alignItems: 'stretch', gap: 6,
      background: t.panel + 'cc', backdropFilter: 'blur(4px)',
      padding: '6px 8px', borderRadius: 5, border: `1px solid ${t.borderStrong}`,
      pointerEvents: 'none', zIndex: 4,
      boxShadow: t.shadow,
    }}>
      <div style={{ position: 'relative', width: w, height: h,
                    border: `1px solid ${t.borderStrong}`, borderRadius: 2, overflow: 'hidden' }}>
        <img src={colormapStripUrl(colormap, 16, 256)} alt={`${colormap} colormap`}
             style={{ width: '100%', height: '100%', display: 'block' }} />
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', justifyContent: 'space-between',
                    fontSize: 9.5, color: t.text, fontFamily: 'ui-monospace,Menlo,monospace',
                    minWidth: 36, textAlign: 'left' }}>
        <span title={`vmax = ${vmax}`}>{fmt(vmax)}</span>
        <span style={{ color: t.textFaint }} title={`midpoint = ${mid}`}>{fmt(mid)}</span>
...
const FloatingWindow = ({ title, icon, x, y, w, h, onChange, onClose, children }) => {
  ...
  const startDrag = (e) => {
    e.preventDefault();
    const sx = e.clientX, sy = e.clientY, x0 = x, y0 = y;
    document.body.style.cursor = 'grabbing';
    const move = (ev) => onChange({ x: Math.max(0, x0 + ev.clientX - sx),
                                     y: Math.max(0, y0 + ev.clientY - sy), w, h });
  ...
  const startResize = (e) => {
    e.preventDefault();
    e.stopPropagation();
    const sx = e.clientX, sy = e.clientY, w0 = w, h0 = h;
    document.body.style.cursor = 'nwse-resize';
    const move = (ev) => onChange({ x, y, w: Math.max(220, w0 + ev.clientX - sx),
  ...
  return (
    <div style={{
      position: 'fixed', left: x, top: y, width: w, height: h, zIndex: 60,
      ...
      <div onMouseDown={startDrag}
        ...
        <span style={{ fontSize: 11, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', flex: 1 }}>{title}</span>
        <button onClick={onClose} title="Dock back into the sidebar"
                style={{ background: 'transparent', border: 'none', color: t.textMuted, cursor: 'pointer', padding: 2, display: 'flex' }}>
    ...
    return (
      <button onClick={onToggle}
              ...
              style={{ padding: '5px 10px', fontSize: 11,
                       ...
                       cursor: 'pointer', fontFamily: 'inherit',
                       display: 'inline-flex', alignItems: 'center', gap: 5 }}>
        <Icon name="palette" size={12} />Style
      </button>
    );
  }

  return (
    <div style={{ padding: '10px 14px', background: t.panelAlt,
      ...
      <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start',
        ...
        <div>
          <div style={{ fontSize: 9.5, color: t.textMuted, textTransform: 'uppercase',
                        letterSpacing: 0.5, fontWeight: 600, marginBottom: 4 }}>Preset</div>
          <div style={{ display: 'flex', gap: 4, flexWrap: 'wrap' }}>
            {Object.keys(PLOT_STYLE_PRESETS).map(k =>
              pillBtn(k[0].toUpperCase() + k.slice(1),
                      () => setStyle({ ...PLOT_STYLE_PRESETS[k] }),
                      false))}
            <button onClick={() => { if (confirm('Reset plot style to defaults?')) resetStyle(); }}
                    style={{ padding: '4px 10px', fontSize: 11, color: t.danger,
                             background: 'transparent', border: `1px solid ${t.chipBorder}`,
                             borderRadius: 5, cursor: 'pointer', fontFamily: 'inherit' }}>
              Reset
            </button>
            <button onClick={onToggle}
                    style={{ padding: '4px 10px', fontSize: 11, color: t.textMuted,
                             background: 'transparent', border: `1px solid ${t.chipBorder}`,
                             borderRadius: 5, cursor: 'pointer', fontFamily: 'inherit' }}>
              Hide
            </button>
        ...
        <div>
          <div style={{ fontSize: 9.5, color: t.textMuted, textTransform: 'uppercase',
                        letterSpacing: 0.5, fontWeight: 600, marginBottom: 4 }}>Typography</div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
            <select value={style.fontFamily}
              ...
              {PLOT_FONT_FAMILIES.map(f => <option key={f.value} value={f.value}>{f.label}</option>)}
            </select>
            <Num label="Title"  value={style.titleSize}     min={8}  max={32} step={0.5}
                 onChange={(v) => setStyle({ titleSize: v })} />
            <Num label="Axis"   value={style.axisLabelSize} min={6}  max={28} step={0.5}
                 onChange={(v) => setStyle({ axisLabelSize: v })} />
            <Num label="Tick"   value={style.tickSize}      min={5}  max={20} step={0.5}
                 onChange={(v) => setStyle({ tickSize: v })} />
            <Num label="Legend" value={style.legendSize}    min={5}  max={22} step={0.5}
                 onChange={(v) => setStyle({ legendSize: v })} />
          </div>
          <div style={{ display: 'flex', gap: 6, marginTop: 4, alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ fontSize: 10, color: t.textFaint }}>weight:</span>
            <select value={style.titleWeight}
                    onChange={(e) => setStyle({ titleWeight: parseInt(e.target.value) })}
                    title="Title font weight"
                    style={{ background: t.inputBg, color: t.text,
                             border: `1px solid ${t.border}`, borderRadius: 3,
                             padding: '2px 4px', fontSize: 11 }}>
              {[300, 400, 500, 600, 700, 800].map(w => <option key={w} value={w}>title {w}</option>)}
            </select>
            <select value={style.axisLabelWeight}
                    onChange={(e) => setStyle({ axisLabelWeight: parseInt(e.target.value) })}
                    title="Axis label font weight"
                    style={{ background: t.inputBg, color: t.text,
                             border: `1px solid ${t.border}`, borderRadius: 3,
                             padding: '2px 4px', fontSize: 11 }}>
              {[300, 400, 500, 600, 700].map(w => <option key={w} value={w}>axis {w}</option>)}
            </select>
            <select value={style.legendWeight}
                    onChange={(e) => setStyle({ legendWeight: parseInt(e.target.value) })}
                    title="Legend font weight"
                    style={{ background: t.inputBg, color: t.text,
                             border: `1px solid ${t.border}`, borderRadius: 3,
                             padding: '2px 4px', fontSize: 11 }}>
              {[300, 400, 500, 600, 700].map(w => <option key={w} value={w}>legend {w}</option>)}
            </select>
            <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4,
                            fontSize: 10.5, color: t.text }}>
              <input type="checkbox" checked={style.titleItalic}
        ...
        <div>
          <div style={{ fontSize: 9.5, color: t.textMuted, textTransform: 'uppercase',
                        letterSpacing: 0.5, fontWeight: 600, marginBottom: 4 }}>Scale</div>
        ...
        <div>
          <div style={{ fontSize: 9.5, color: t.textMuted, textTransform: 'uppercase',
                        letterSpacing: 0.5, fontWeight: 600, marginBottom: 4 }}>Lines</div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <Num label="Line"   value={style.lineWidth}        min={0.5} max={6}  step={0.1}
                 onChange={(v) => setStyle({ lineWidth: v })} />
            <Num label="Marker" value={style.markerSize}       min={2}   max={16} step={0.5}
                 onChange={(v) => setStyle({ markerSize: v })} />
            <Num label="Axis"   value={style.axisStrokeWidth}  min={0.2} max={3}  step={0.1}
        ...
        <div>
          <div style={{ fontSize: 9.5, color: t.textMuted, textTransform: 'uppercase',
                        letterSpacing: 0.5, fontWeight: 600, marginBottom: 4 }}>Grid</div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4,
                            fontSize: 10.5, color: t.text }}>
              <input type="checkbox" checked={style.showGrid}
                     onChange={(e) => setStyle({ showGrid: e.target.checked })}
                     style={{ margin: 0 }} />
              show
            </label>
            <Num label="Width"   value={style.gridWidth}   min={0.1} max={2}  step={0.1}
                 onChange={(v) => setStyle({ gridWidth: v })} />
            <Num label="Opacity" value={style.gridOpacity} min={0}   max={1}  step={0.05}
        ...
        <div>
          ...
          <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4,
                            fontSize: 10.5, color: t.text }}>
              <input type="checkbox" checked={style.cardBorder}
                     onChange={(e) => setStyle({ cardBorder: e.target.checked })}
                     style={{ margin: 0 }} />
              border
            </label>
            <Num label="Radius"  value={style.cardBorderRadius} min={0} max={24} step={1}
        ...
        <div>
          <div style={{ fontSize: 9.5, color: t.textMuted, textTransform: 'uppercase',
                        letterSpacing: 0.5, fontWeight: 600, marginBottom: 4 }}>Palette</div>
          <select value={style.palette}
        ...
        <div>
          <div style={{ fontSize: 9.5, color: t.textMuted, textTransform: 'uppercase',
                        letterSpacing: 0.5, fontWeight: 600, marginBottom: 4 }}>Export</div>
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <select value={style.exportScale}
            ...
            <select value={style.exportFormat}
            ...
            <select value={style.exportBackground}
  ...
  return (
    <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6 }}>
      ...
      <Tip title="Rearranges the plots in this tab into the chosen grid. Useful before you hit Export PNG — dom-to-image captures whatever layout is on screen.">
        <Segmented value={value} options={options.map(o => ({ value: o.id, label: o.label }))}
                   onChange={onChange} />
      </Tip>
    </div>
  );
...
// ---------------------------------------------------------------------------
const ResizeHandle = ({ value, onChange, min = 200, max = 800, side = 'right',
                        grow = 1, width = 6 }) => {
  const t = useTheme();
  const [hover, setHover] = useState(false);
  const [dragging, setDragging] = useState(false);
  ...
  const pos = side === 'right' ? { right: -Math.floor(width / 2) }
                               : { left:  -Math.floor(width / 2) };
  const bg = dragging ? t.accent
           : hover    ? t.accent + '66'
           : 'transparent';
  return (
    <div
      ...
      style={{
        position: 'absolute', top: 0, bottom: 0, ...pos, width,
        cursor: 'col-resize', zIndex: 5, background: bg,

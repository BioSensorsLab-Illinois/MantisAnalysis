...
const sCycTag = (s) => s < 3 ? 'Below Nyquist — not trustworthy'
                    : s < 5 ? 'Marginal sampling'
                    : 'Well sampled';
...
const USAFMode = ({ onRunAnalysis, onStatusChange, say, onSwitchSource, onOpenFile }) => {
  ...
  const defaultCh = available.includes('LG-G') ? 'LG-G'
                  : available.includes('HG-G') ? 'HG-G'
  ...
  const DEFAULT_LEFT_ORDER  = ['source', 'displayChannel', 'analysisChannels', 'picking', 'outputMode'];
  ...
  const [imgDims, setImgDims] = useStateU({ w: source?.shape?.[1] || 720, h: source?.shape?.[0] || 540 });
  ...
  const measureOne = useCallbackU(async (spec, manualBars, manualGaps) => {
    if (!source) return null;
    try {
      const body = {
        source_id: source.source_id, channel: activeChannel,
        line: spec, swath_width: 8.0, method, isp: buildIspPayload(),
      };
    ...
    } catch (err) { say?.(`measure failed: ${err.detail || err.message}`, 'danger'); return null; }
  }, [source, activeChannel, method, ispEnabled, ispLive, ispSharp, ispRadius, ispDenoise, ispBlackLvl, ispMethod]);

  const advance = () => {
    if (!autoInc && !seqMode) return;
    let ne = element + 1, ng = group;
    if (ne > 6) { ne = 1; if (seqMode) ng = Math.min(5, group + 1); }
    setElement(ne); setGroup(ng);
  };
  ...
  const addLine = async (p0, p1) => {
    ...
    const dx = Math.abs(snapped[0] - p0[0]), dy = Math.abs(snapped[1] - p0[1]);
  ...
  const updateLinePoints = useCallbackU((lineId, nextBars, nextGaps) => {
    setLines(prev => prev.map(l => l.id === lineId
      ? { ...l, manualBars: nextBars, manualGaps: nextGaps, pending: true }
      : l));
    const line = lines.find(l => l.id === lineId);
    if (!line) return;
    const spec = { group: line.group, element: line.element, direction: line.direction, p0: line.p0, p1: line.p1 };
  ...
  useEffectU(() => {
    ...
    (async () => {
      const updated = await Promise.all(lines.map(async l => {
        const m = await measureOne({ group: l.group, element: l.element, direction: l.direction, p0: l.p0, p1: l.p1 });
  ...
  }, [ispEnabled, ispLive, ispSharp, ispRadius, ispDenoise, ispBlackLvl, ispMethod, method, activeChannel]);
  ...
  const toImg = useCallbackU((ev) => {
    ...
    if (imgAR > innerAR) { renderedW = r.width;  renderedH = r.width / imgAR; }
    else                 { renderedH = r.height; renderedW = r.height * imgAR; }
  ...
  const onCanvasDown = (ev) => {
    ...
    const onUp = (e) => {
      ...
      if (dragged) { addLine(p, toImg(e)); setFirstClick(null); setHoverPos(null); }
      else if (firstClick) { addLine(firstClick, p); setFirstClick(null); setHoverPos(null); }
      else { setFirstClick(p); }
    };
  ...
  const onCanvasRight = (ev) => {
    ...
    if (best) {
      ...
      setSelectedIds(prev => { const n = new Set(prev); n.delete(best.id); return n; });
    }
  };
  ...
  useEffectU(() => {
    const down = (e) => {
      ...
      if (e.key === 'z' && (e.ctrlKey || e.metaKey)) { undoLastLine(); e.preventDefault(); return; }
      if (e.key === 'a' && (e.ctrlKey || e.metaKey)) { setSelectedIds(new Set(lines.map(l => l.id))); e.preventDefault(); return; }
      ...
      if (e.key === 'f' || e.key === 'F') { setZoom(1); setPan([0, 0]); }
  ...
  const sortedLines = useMemoU(() => {
    ...
    const getV = {
      id: l => l.id, g: l => l.group, e: l => l.element, d: l => l.direction,
    ...
    copy.sort((a, b) => { const va = getV(a), vb = getV(b); return (va > vb ? 1 : va < vb ? -1 : 0) * dir; });
    return copy;
  }, [lines, sortCol, sortDir]);
  const setSort = (c) => { if (sortCol === c) setSortDir(d => d === 'asc' ? 'desc' : 'asc'); else { setSortCol(c); setSortDir('asc'); } };

  // ---- Status bar --------------------------------------------------------
  useEffectU(() => {
    onStatusChange?.(`${lines.length} lines · G${group}E${element}${direction} (${nextLpmm.toFixed(2)} lp/mm)`, lines.length);
  ...
  const exportConfig = () => {
    const cfg = {
      kind: 'mantis-usaf-config', version: 3, exportedAt: new Date().toISOString(),
      ...
      dark: source?.has_dark
        ...
        : null,
      picker: { activeChannel, group, element, direction, autoInc, seqMode, method, thresholdPct, snap },
      view: { rotation, flipH, flipV, zoom, brightness, contrast, gamma, colormap },
      isp: { enabled: ispEnabled, live: ispLive, method: ispMethod, sharp: ispSharp, radius: ispRadius, denoise: ispDenoise, blackLvl: ispBlackLvl },
      lines: lines.map(l => ({ id: l.id, group: l.group, element: l.element, direction: l.direction, p0: l.p0, p1: l.p1 })),
      selectedIds: [...selectedIds], sortCol, sortDir,
    };
    exportJSON(`mantis-usaf-${Date.now()}.json`, cfg);
    say?.(`Config saved${source?.path ? ' (with H5 path)' : ' (without H5 path — browser upload)'}`, 'success');
  };
  const importConfig = async (file) => {
    ...
    try {
      ...
      if (cfgPath && cfgPath !== source?.path) {
        try {
          const newSrc = await apiFetch('/api/sources/load-path', { method: 'POST', body: { path: cfgPath } });
          onSwitchSource?.(newSrc);
          activeSrc = newSrc;
      ...
      if (cfg.dark?.path && activeSrc) {
        ...
        say?.(`Config references dark "${cfg.dark.name}" — re-attach via the Dark frame card`, 'warn');
      }
      ...
      if (Array.isArray(cfg.lines)) {
        ...
        const measured = await Promise.all(placeholder.map(async l => {
          const m = await measureOne({ group: l.group, element: l.element, direction: l.direction, p0: l.p0, p1: l.p1 });
          return { ...l, m, pending: false };
        }));
        setLines(measured);
      ...
      say?.(`Loaded ${cfg.lines?.length ?? 0} line${cfg.lines?.length === 1 ? '' : 's'} from ${file.name}`, 'success');
    } catch (err) { say?.(`Load failed: ${err.message}`, 'danger'); }
  };

  const exportCSVTable = () => {
    const rows = sortedLines.filter(l => l.m).map(l => ({
      id: l.id, group: l.group, element: l.element, direction: l.direction,
      ...
      n_samples: l.m.n_samples, reliability: l.m.reliability,
      line_length_px: l.m.line_length_px.toFixed(3),
    }));
    if (!rows.length) { say?.('Nothing to export — no measured lines.', 'warn'); return; }
  ...
  const runAnalysis = async () => {
    ...
    try {
      say?.(`Running USAF analysis on ${analysisChannels.length} channel${analysisChannels.length !== 1 ? 's' : ''}…`);
      const body = {
        source_id: source.source_id, channels: analysisChannels,
        lines: lines.map(l => ({ group: l.group, element: l.element, direction: l.direction, p0: l.p0, p1: l.p1 })),
        threshold,
        transform: { rotation, flip_h: flipH, flip_v: flipV },
        isp: buildIspPayload(),
      };
      const res = await apiFetch('/api/usaf/analyze', { method: 'POST', body });
      onRunAnalysis({ mode: 'usaf', source, channels: analysisChannels, lines, threshold, response: res });
  ...
  const profilePreviewBody = (
    <ProfilePreview
      line={selectedLine} method={method} multiCount={selectedIds.size}
      threshold={threshold} ispApplied={ispEnabled && ispLive}
  ...
  const leftPanels = {
    source: (
      <Card title="Source" icon="open" pinned>
          <Tip title="Currently loaded recording">
            <div style={{ fontSize: 11.5, color: t.text, wordBreak: 'break-all', fontFamily: 'ui-monospace,Menlo,monospace' }}>
              {source?.name || '(none)'}
            </div>
          </Tip>
          <div style={{ fontSize: 10.5, color: t.textFaint, marginTop: 3, fontFamily: 'ui-monospace,Menlo,monospace' }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginTop: 8 }}>
            <Button variant="ghost" icon="save" size="xs" onClick={exportConfig}
                    title="Save picker / view / ISP / all picked lines to JSON" fullWidth>Save cfg</Button>
            <Button variant="ghost" icon="upload" size="xs" onClick={() => fileInputRef.current?.click()}
                    title="Load a previously-saved JSON; lines will be re-measured against the current source" fullWidth>Load cfg</Button>
          </div>
          ...
          <div style={{ marginTop: 10, padding: '6px 8px', background: source?.has_dark ? t.accentSoft : t.panelAlt, border: `1px solid ${source?.has_dark ? t.accent + '33' : t.border}`, borderRadius: 4 }}>
            <div style={{ fontSize: 10, color: source?.has_dark ? t.accent : t.textMuted, fontWeight: 600, letterSpacing: 0.4, textTransform: 'uppercase', display: 'flex', alignItems: 'center', gap: 6 }}>
              <Icon name="layers" size={11} />
              <span>Dark frame</span>
              {source?.has_dark && <span style={{ marginLeft: 'auto', width: 6, height: 6, borderRadius: '50%', background: t.success }} />}
            </div>
            <div style={{ fontSize: 11, color: source?.has_dark ? t.text : t.textFaint, marginTop: 4, fontFamily: 'ui-monospace,Menlo,monospace', wordBreak: 'break-all' }}>
              {source?.has_dark ? `subtracted: ${source.dark_name}` : 'not attached — analysis uses raw DN'}
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 6, marginTop: 6 }}>
              <Button variant="ghost" icon="open" size="xs" onClick={() => darkInputRef.current?.click()}
                      title={`Pick a dark file to subtract per-pixel before analysis · current filter: ${fileFilter.filters[fileFilter.current]?.label}`} fullWidth>{source?.has_dark ? 'Replace' : 'Load'}</Button>
              <Button variant="ghost" icon="open" size="xs" onClick={onLoadDarkByPath}
                      title="Type / paste an absolute disk path. Path is saved with cfg so Load cfg auto-attaches this dark later." fullWidth>by path…</Button>
              <Button variant="danger" icon="trash" size="xs" disabled={!source?.has_dark} onClick={onClearDark}
                      title="Detach the dark frame; analysis returns to raw DN" fullWidth>Clear</Button>
            </div>
    ...
    displayChannel: (
      <Card title="Display channel" icon="layers">
          <div style={{ fontSize: 10.5, color: t.textFaint, marginBottom: 6 }}>Sensor readout shown in canvas</div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
            {available.map(c => (
              <Tip key={c} title={`View ${c}`}>
                <ChannelChip id={chipId(c)} selected={activeChannel === c} onToggle={() => setActiveChannel(c)} size="sm" />
              </Tip>
            ))}
          </div>
        </Card>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 4 }}>
            {available.map(c => (
              <Tip key={c} title={`Include ${c} in Run analysis`}>
                <ChannelChip id={chipId(c)} multi selected={analysisChannels.includes(c)}
                  onToggle={() => setAnalysisChannels(prev => prev.includes(c) ? prev.filter(x => x !== c) : [...prev, c])}
                  size="sm" />
              </Tip>
            ))}
          </div>
          <div style={{ display: 'flex', gap: 6, marginTop: 8 }}>
            <Button size="xs" title="Select every channel for analysis" onClick={() => setAnalysisChannels(available)}>All</Button>
            <Button size="xs" title="Clear all selected channels" onClick={() => setAnalysisChannels([])}>None</Button>
            {available.some(c => c.startsWith('HG-')) && <>
              <Button size="xs" title="Select only the high-gain (HG-*) channels" onClick={() => setAnalysisChannels(available.filter(c => c.startsWith('HG-')))}>HG</Button>
              <Button size="xs" title="Select only the low-gain (LG-*) channels" onClick={() => setAnalysisChannels(available.filter(c => c.startsWith('LG-')))}>LG</Button>
            </>}
          </div>
        </Card>
    ),
    ...
    picking: (
      <Card title="Picking" icon="pin" pinned>
          <Row label="Group"><Tip title="USAF group number (0–5). ← / → also changes this."><Spinbox value={group} min={0} max={5} onChange={setGroup} /></Tip><div style={{ flex: 1 }} /></Row>
          <Row label="Element"><Tip title="USAF element (1–6). ↑ / ↓ also changes this."><Spinbox value={element} min={1} max={6} onChange={setElement} /></Tip><div style={{ flex: 1 }} /></Row>
          <Row label="Direction"><Tip title="Bar orientation. Auto-set from stroke direction when you draw."><Segmented value={direction} options={['H', 'V']} onChange={setDirection} /></Tip></Row>
          <div style={{ marginTop: 10, padding: '8px 10px', background: t.accentSoft, borderRadius: 5, border: `1px solid ${t.accent}22` }}>
            <div style={{ fontSize: 10, color: t.accent, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase' }}>Next stroke</div>
            <div style={{ marginTop: 3, display: 'flex', alignItems: 'baseline', gap: 8 }}>
              <span style={{ fontSize: 18, fontWeight: 600, color: t.text, fontFamily: 'ui-monospace,Menlo,monospace' }}>G{group}E{element}{direction}</span>
              <span style={{ fontSize: 11, color: t.textMuted, fontFamily: 'ui-monospace,Menlo,monospace' }}>{nextLpmm.toFixed(2)} lp/mm</span>
              <span style={{ marginLeft: 'auto', width: 10, height: 10, borderRadius: '50%', background: usafColorForElt(element) }} />
            </div>
          </div>
          ...
          <Tip title="After each picked line, auto-increment Element by 1 so consecutive picks march through E1 → E6 without manual clicks. Off = element stays put after each pick."><Checkbox checked={autoInc} onChange={setAutoInc} label="Auto-increment after pick" /></Tip>
          <Tip title="When Element rolls past 6, advance Group by 1 and reset Element to 1, so picks sweep G0E1, G0E2, …, G0E6, G1E1, G1E2, … (also forces auto-increment on)."><Checkbox checked={seqMode} onChange={(v) => { setSeqMode(v); if (v) setAutoInc(true); }} label="Sequential G+E sweep" /></Tip>
          <Tip title="When drawing, snap the stroke to a perfect horizontal or vertical axis if it's within 8 pixels of one. Helpful for USAF where bars are axis-aligned. Off = freehand."><Checkbox checked={snap} onChange={setSnap} label="Snap to axis" /></Tip>
          <div style={{ display: 'flex', gap: 6, marginTop: 10 }}>
            <Tip title="Reset Group + Element back to G0 / E1 (the lowest-frequency element on a standard USAF target)"><Button onClick={() => { setGroup(0); setElement(1); }} size="xs">Reset G0 E1</Button></Tip>
            <Tip title="Reset canvas zoom to 100% and pan to center"><Button icon="zoomReset" size="xs" onClick={() => { setZoom(1); setPan([0, 0]); }}>Reset zoom</Button></Tip>
          </div>
        </Card>
    ),
      ...
      <Card title="Output mode" icon="run" pinned>
          <Row label="Threshold">
            <Tip title="MTF pass/fail threshold. Typical: 30% (CTF) or 10% (Rayleigh).">
              <Segmented value={`${thresholdPct}%`} options={['50%', '30%', '20%', '10%']}
                         onChange={v => setThresholdPct(parseInt(v))} fullWidth />
            </Tip>
          </Row>
          <div style={{ marginTop: 12 }}>
            <Tip title="POST to /api/usaf/analyze. Matplotlib PNGs come back from the server.">
              <Button variant="primary" icon="run" size="lg" fullWidth disabled={lines.length === 0 || analysisChannels.length === 0} onClick={runAnalysis}>
                Run analysis
              </Button>
            </Tip>
            <div style={{ fontSize: 10.5, color: t.textFaint, marginTop: 6, textAlign: 'center' }}>
              {lines.length === 0 ? 'Pick at least one line.' :
               analysisChannels.length === 0 ? 'Choose ≥1 analysis channel.' :
  ...
  const togglePopOut = (e) => { e.stopPropagation(); setProfileFloating(f => !f); };
  const popOutBtn = (
    <Tip title={profileFloating ? 'Dock back into the sidebar' : 'Pop out as a floating window you can place anywhere on screen'}>
      <span role="button" tabIndex={0}
            onMouseDown={(e) => e.stopPropagation()}
            onClick={togglePopOut}
            onKeyDown={(e) => { if (e.key === 'Enter' || e.key === ' ') togglePopOut(e); }}
            style={{ color: t.textMuted, cursor: 'pointer', padding: 2,
                     display: 'inline-flex', marginRight: 4, borderRadius: 3 }}>
  ...
  const rightPanels = {
    display: (
      <Card title="Display" icon="sliders">
        ...
        <Tip title="Display-only brightness offset (CSS filter). Does NOT affect what the analysis math sees — change ISP black level for that."><Slider label="Brightness" min={-0.5} max={0.5} step={0.01} value={brightness} onChange={setBrightness} /></Tip>
        <Tip title="Display-only contrast multiplier (CSS filter). Stretches mid-range visually; analysis is unaffected."><Slider label="Contrast" min={0.5} max={2.5} step={0.01} value={contrast} onChange={setContrast} /></Tip>
        <Tip title="Display-only gamma curve (visualization only — analysis uses raw DN)."><Slider label="Gamma" min={0.4} max={2.5} step={0.01} value={gamma} onChange={setGamma} /></Tip>
        ...
        <div style={{ marginTop: 10, padding: '6px 8px', background: t.panelAlt,
          ...
          <div style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
            ...
            <Tip title={autoRange ? 'Click to pin vmin/vmax to current values'
              ...
              <button onClick={() => {
                if (autoRange) { setAutoRange(false); /* values already correct */ }
        ...
        <div style={{ fontSize: 10, color: t.textFaint, marginTop: 6, lineHeight: 1.4 }}>
          Display only — does not affect analysis. Use the ISP card below to change what the server measures.
        </div>
      </Card>
    ),
    ...
    linesTable: (
      <LinesTable
        lines={sortedLines} selectedIds={selectedIds} toggleSel={toggleSel}
        ...
        onClearAll={() => {
          ...
          const first = lines[0];
          if (first?.prior) { setGroup(first.prior.group); setElement(first.prior.element); setDirection(first.prior.direction); }
          setLines([]);
          setSelectedIds(new Set());
        }}
    ...
    profilePreview: (
      <Card title="Profile preview" icon="eye" pinned actions={popOutBtn}>
        {profileFloating ? (
          <div style={{ fontSize: 11, color: t.textFaint, padding: '12px 4px', lineHeight: 1.5,
                        textAlign: 'center', border: `1px dashed ${t.border}`, borderRadius: 4 }}>
            Profile preview is floating in its own window.<br/>
    ...
    summary: (
      <Card title="Summary" icon="pin">
        {(() => {
          ...
          const failing = measured.filter(l => l.m.modulation_5pt < threshold).sort((a, b) => a.m.lp_mm - b.m.lp_mm);
          ...
          return (
            <StatBlock
              ...
              items={[
                ...
                { label: 'pass',             value: `${passCount}`, color: passCount > 0 ? t.success : t.textMuted },
                { label: 'fail',             value: `${failing.length}`, color: failing.length === 0 ? t.success : t.danger, muted: failing.length === 0 },
                { label: 'detection limit',  key: 'lim',
                  value: firstFail ? `${firstFail.m.lp_mm.toFixed(2)} lp/mm` : 'above all picks',
                  color: firstFail ? t.accent : t.success },
                { label: 'first failure',    value: firstFail ? `G${firstFail.group}E${firstFail.element}${firstFail.direction}` : '—', muted: !firstFail },
              ]}
            />
          );
        })()}
  ...
  return (
    <div style={{ display: 'grid',
      ...
      {/* ================================================================= LEFT */}
      <div style={{ position: 'relative', borderRight: `1px solid ${t.border}`, background: t.bg, padding: 10, overflowY: 'auto' }}>
      ...
      <USAFCanvas
        ...
        lines={lines} selectedIds={selectedIds} firstClick={firstClick} hoverPos={hoverPos}
        cursor={cursorReadout}
        group={group} element={element} direction={direction}
        rotation={rotation} flipH={flipH} flipV={flipV} zoom={zoom} pan={pan} snap={snap}
        ...
        threshold={threshold} passCount={passCount}
        colormap={colormap} vmin={vmin} vmax={vmax}
      />

      {/* ================================================================= RIGHT */}
      <div style={{ position: 'relative', borderLeft: `1px solid ${t.border}`, background: t.bg, padding: 10, overflowY: 'auto' }}>
        <ResizeHandle value={rightW} onChange={setRightW} min={260} max={680} side="left" grow={-1} />
      ...
      {profileFloating && (
        <FloatingWindow title="Profile preview" icon="eye"
          x={profileWin.x} y={profileWin.y} w={profileWin.w} h={profileWin.h}
...
const USAFCanvas = ({
  ...
  lines, selectedIds, firstClick, hoverPos, cursor,
  group, element, direction,
  rotation, flipH, flipV, zoom, pan, snap,
  ...
  return (
    <div style={{ background: t.bg, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
      {/* Canvas header */}
      <div style={{ padding: '10px 14px', display: 'flex', alignItems: 'center', borderBottom: `1px solid ${t.border}`, background: t.bg }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 500, color: t.text, display: 'flex', alignItems: 'center', gap: 8 }}>
            <span style={{ width: 10, height: 10, borderRadius: '50%', background: CHANNEL_COLORS[parseChannel(channel?.includes('-') ? channel : `HG-${channel}`).band] || t.accent }} />
            {channel}
          </div>
          <div style={{ fontSize: 10.5, color: t.textFaint, marginTop: 2, fontFamily: 'ui-monospace,Menlo,monospace' }}>
            USAF 1951 · rot {rotation}°{flipH ? ' · H' : ''}{flipV ? ' · V' : ''} · zoom {(zoom * 100).toFixed(0)}%
          </div>
        </div>
        <div style={{ marginLeft: 'auto', display: 'flex', gap: 16, alignItems: 'center', fontFamily: 'ui-monospace,Menlo,monospace', fontSize: 11.5, color: t.textMuted }}>
          {cursor && <span title="Cursor image-space coordinates">x={String(cursor[0]).padStart(4, '0')} · y={String(cursor[1]).padStart(4, '0')}</span>}
          <span style={{ color: t.textFaint }}>{lines.length} picked · {passCount} pass</span>
        </div>
      </div>
      ...
      <div style={{ flex: 1, position: 'relative', background: t.canvasBg, overflow: 'hidden' }}>
        ...
        <div style={{ position: 'absolute', top: 0, left: 0, width: 26, height: 20, background: t.panelAlt,
                      borderRight: `1px solid ${t.border}`, borderBottom: `1px solid ${t.border}`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      color: t.textFaint, fontSize: 9, fontFamily: 'ui-monospace,Menlo,monospace' }}>px</div>
        ...
        <div
          ...
          onWheel={onWheel}
          style={{ position: 'absolute', top: 20, left: 26, right: 0, bottom: 0, cursor: canvasCursor, userSelect: 'none' }}
        >
          ...
          <div style={{ position: 'absolute', inset: 0, transform: innerTx, transformOrigin: 'center', transition: 'none', pointerEvents: 'none' }}>
          ...
          <svg width="100%" height="100%" style={{ position: 'absolute', inset: 0, pointerEvents: 'none', overflow: 'visible' }}>
            {(() => {
              const STROKE = 1.6, STROKE_SEL = 2.4, STROKE_GUIDE = 0.8;
              const DOT_R = 3.2, DOT_STROKE = 0.6;
              const HALO = 8, CROSS_ARM = 7, CROSS_STROKE = 1.5;
              const children = [];
              ...
              if (firstClick && hoverPos && snap) {
                ...
                children.push(
                  <g key="snap" opacity={0.4}>
                    <line x1={0} y1={fy} x2={cRect.w} y2={fy} stroke="#4a9eff" strokeWidth={STROKE_GUIDE} strokeDasharray="3 4" />
                    <line x1={fx} y1={0} x2={fx} y2={cRect.h} stroke="#4a9eff" strokeWidth={STROKE_GUIDE} strokeDasharray="3 4" />
                  </g>
                );
              }
              // Lines + labels.
              lines.forEach(l => {
                ...
                children.push(
                  <g key={l.id}>
                    {sel && <line x1={x0} y1={y0} x2={x1} y2={y1} stroke="#ffd54f" strokeWidth={HALO} strokeLinecap="round" opacity={0.55} />}
                    <line x1={x0} y1={y0} x2={x1} y2={y1}
                          stroke={c} strokeWidth={sel ? STROKE_SEL : STROKE} strokeLinecap="round"
                          opacity={l.pending ? 0.5 : 1}
                          strokeDasharray={pass === false ? '6 3' : ''} />
                    <circle cx={x0} cy={y0} r={DOT_R} fill={c} stroke="#000" strokeWidth={DOT_STROKE} />
                    <circle cx={x1} cy={y1} r={DOT_R} fill={c} stroke="#000" strokeWidth={DOT_STROKE} />
                    {/* Label: screen-space, always upright */}
                    <g transform={`translate(${mx}, ${my - 16})`}>
                      <rect x={-22} y={-8} width={44} height={15} rx={3} fill="rgba(10,12,16,0.92)"
                            stroke={sel ? '#ffd54f' : c} strokeWidth={sel ? 1.3 : 0.7} />
                      <text x={0} y={3} textAnchor="middle" fill="#fff" fontSize={10}
                            fontFamily="ui-monospace,Menlo,monospace" fontWeight={500}>
                        G{l.group}E{l.element}{l.direction}
                      </text>
                    </g>
                  </g>
                );
              ...
              if (firstClick) {
                ...
                children.push(
                  <g key="first">
                    <line x1={fx - CROSS_ARM} y1={fy} x2={fx + CROSS_ARM} y2={fy} stroke="#e5484d" strokeWidth={CROSS_STROKE} />
                    <line x1={fx} y1={fy - CROSS_ARM} x2={fx} y2={fy + CROSS_ARM} stroke="#e5484d" strokeWidth={CROSS_STROKE} />
                    {p && <line x1={fx} y1={fy} x2={p[0]} y2={p[1]} stroke="#e5484d" strokeWidth={CROSS_STROKE * 0.85} strokeDasharray="4 3" opacity={0.85} />}
                  </g>
                );
              }
              return children;
          ...
          {(firstClick || hoverPos) && (() => {
            const [sx, sy] = imgToScreen((hoverPos || firstClick)[0], (hoverPos || firstClick)[1]);
            return (
              <HUD style={{ left: `${sx}px`, top: `${sy}px`, transform: 'translate(14px, -34px)' }}>
                <span style={{ color: usafColorForElt(element) }}>● </span>
                G{group}E{element}{direction}
                <span style={{ opacity: 0.7, marginLeft: 6 }}>{usafLpmm(group, element).toFixed(2)} lp/mm</span>
                {firstClick && hoverPos && (
                  <span style={{ opacity: 0.7, marginLeft: 6 }}>
                    · L={Math.round(Math.hypot(hoverPos[0] - firstClick[0], hoverPos[1] - firstClick[1]))}px
                  </span>
                )}
              </HUD>
            );
        ...
        <CanvasToolbar position="top-left">
          <CanvasBtn icon="crosshair" active={tool === 'pick' && !spacePan} onClick={() => setTool('pick')} title="Pick tool — draw & select lines" />
          <CanvasBtn icon="hand" active={tool === 'pan' || spacePan} onClick={() => setTool('pan')} title="Pan tool (or hold Space) — drag the canvas" />
          ...
          <CanvasBtn icon="zoomReset" label={`${Math.round(zoom * 100)}%`} onClick={onZoomReset} title="Reset zoom + pan (0)" />
          <CanvasBtn icon="fit" onClick={onZoomReset} title="Fit to view (F)" />
          <CanvasBtn icon="undo" onClick={onUndo} title="Undo last line (⌘Z)" />
        </CanvasToolbar>

        {/* Hint strip */}
        <div style={{ position: 'absolute', bottom: 10, left: 26, right: 0, display: 'flex', justifyContent: 'center', pointerEvents: 'none' }}>
          <div style={{ padding: '5px 12px', background: 'rgba(10,10,10,0.68)', backdropFilter: 'blur(6px)', borderRadius: 20, fontSize: 10.5, color: '#aab3bf', border: '1px solid rgba(255,255,255,0.05)' }}>
            <Kbd tone="dim">drag</Kbd> line ·
            <Kbd tone="dim">space</Kbd> pan ·
            <Kbd tone="dim">wheel</Kbd> zoom ·
            <Kbd tone="dim">rmb</Kbd> delete ·
            <Kbd tone="dim">←→↑↓</Kbd> G/E ·
...
// ===========================================================================
const ISPCard = ({ enabled, setEnabled, live, setLive, method, setMethod,
                   sharp, setSharp, radius, setRadius, denoise, setDenoise,
                   blackLvl, setBlackLvl }) => {
  const t = useTheme();
  const actions = (
    <div onClick={(e) => e.stopPropagation()} style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
      <Tip title="Enable / disable ISP pipeline server-side">
        <label style={{ display: 'inline-flex', alignItems: 'center', gap: 4, cursor: 'pointer', fontSize: 10, color: enabled ? t.accent : t.textMuted }}>
          <input type="checkbox" checked={enabled} onChange={(e) => setEnabled(e.target.checked)} style={{ margin: 0, cursor: 'pointer' }} />
          {enabled ? 'ON' : 'OFF'}
        </label>
      </Tip>
    </div>
  ...
  return (
    <Card title="ISP" icon="sparkles" pinned actions={actions}>
      <div style={{
        padding: '6px 8px', background: enabled ? t.accentSoft : t.panelAlt,
        border: `1px solid ${enabled ? t.accent + '33' : t.border}`, borderRadius: 5,
        display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8,
      }}>
        <Tip title="When checked, ISP params flow into every /api/usaf/measure + /api/usaf/analyze call.">
          <label style={{ display: 'flex', alignItems: 'center', gap: 6, cursor: enabled ? 'pointer' : 'not-allowed', fontSize: 11, color: enabled ? t.text : t.textFaint }}>
            <input type="checkbox" checked={live} disabled={!enabled} onChange={(e) => setLive(e.target.checked)} style={{ margin: 0, cursor: 'inherit' }} />
            <span style={{ fontWeight: 500 }}>Apply live</span>
          </label>
        </Tip>
        <span style={{ fontSize: 9.5, color: t.textFaint, fontFamily: 'ui-monospace,Menlo,monospace' }}>→ server measure + analyze</span>
        {enabled && live && <span style={{ marginLeft: 'auto', width: 7, height: 7, borderRadius: '50%', background: t.accent, boxShadow: `0 0 0 3px ${t.accent}33` }} />}
      </div>
      <div style={{ opacity: !enabled ? 0.45 : 1, pointerEvents: !enabled ? 'none' : 'auto' }}>
        <Row label="Method"><Tip title="Sharpening algorithm applied by the Python ISP pipeline."><Select value={method} onChange={setMethod} options={['Unsharp mask', 'Laplacian', 'High-pass', 'None']} /></Tip></Row>
        <Tip title="How aggressively the sharpening filter pushes high-frequency detail. 0 = no sharpening; >1.0 may over-shoot edges."><Slider label="Sharpen amount" min={0} max={1.5} step={0.05} value={sharp} onChange={setSharp} /></Tip>
        <Tip title="Sharpening kernel radius in pixels. Smaller = enhance fine detail; larger = enhance broader features."><Slider label="Radius (px)"    min={0.4} max={4} step={0.1} value={radius} onChange={setRadius} format={v => v.toFixed(1)} /></Tip>
        <Tip title="Gaussian blur σ applied AFTER sharpening to suppress noise. 0 = no denoise."><Slider label="Denoise (σ)"    min={0} max={1} step={0.05} value={denoise} onChange={setDenoise} /></Tip>
        <Tip title="Constant DN offset subtracted from every pixel before measurement (e.g. to remove bias-frame pedestal)."><Slider label="Black level"    min={0} max={2000} step={25} value={blackLvl} onChange={setBlackLvl} format={v => v.toFixed(0)} /></Tip>
        <div style={{ display: 'flex', gap: 6, marginTop: 10, flexWrap: 'wrap' }}>
          <Tip title="Reset ISP to mild defaults"><Button size="xs" onClick={() => { setMethod('Unsharp mask'); setSharp(0.4); setRadius(1.2); setDenoise(0.2); setBlackLvl(0); }}>Reset</Button></Tip>
          <Tip title="Zero everything — pure raw"><Button size="xs" onClick={() => { setSharp(0); setDenoise(0); setBlackLvl(0); }}>Bypass</Button></Tip>
          <div style={{ flex: 1 }} />
          <Tip title="Aggressive sharpen preset for MTF enhancement"><Button size="xs" icon="sparkles" onClick={() => { setMethod('Unsharp mask'); setSharp(0.8); setRadius(1.6); setDenoise(0.35); }}>MTF+</Button></Tip>
        </div>
      </div>
    </Card>
  );
...
// ===========================================================================
const LinesTable = ({ lines, selectedIds, toggleSel, sortCol, sortDir, setSort,
                      threshold, ispApplied, onDelete, onSelectAll, onClearAll, onCSV, totalLines }) => {
  const t = useTheme();
  ...
  const cols = [
    ...
    { id: 'mich', label: 'Mich (5-pt)', w: 70, title: '5-point Michelson contrast — robust against percentile artifacts' },
  ...
  return (
    <Card title={`Picked lines · ${totalLines}${selectedIds.size ? ` (${selectedIds.size} sel)` : ''}`} icon="grid" pinned>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, padding: '4px 2px 8px', fontSize: 10, color: t.textMuted, fontFamily: 'ui-monospace,Menlo,monospace' }}>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: t.success }} /> pass ≥{(threshold * 100).toFixed(0)}%</span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: t.danger }} /> fail</span>
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}><span style={{ width: 8, height: 8, borderRadius: 2, background: '#d97706' }} /> &lt; Nyquist</span>
        {ispApplied && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, color: t.accent, marginLeft: 'auto' }}><Icon name="sparkles" size={10} /> ISP live</span>}
      </div>
      <div style={{ border: `1px solid ${t.border}`, borderRadius: 5, overflow: 'hidden' }}>
        <div style={{
          display: 'grid', gridTemplateColumns: cg, gap: 0, padding: '6px 10px',
          fontSize: 9.5, color: t.textMuted, textTransform: 'uppercase', letterSpacing: 0.4, fontWeight: 600,
          background: t.panelAlt, borderBottom: `1px solid ${t.border}`,
        }}>
          <Tip title="Select / deselect all"><input type="checkbox" checked={allSelected} onChange={(e) => e.target.checked ? onSelectAll() : toggleSel(-1, {})} style={{ margin: 0, cursor: 'pointer' }} /></Tip>
          {cols.map(c => (
            <Tip key={c.id} title={`Sort by ${c.title}`}>
              <div onClick={() => setSort(c.id)}
        ...
        <div style={{ maxHeight: 260, overflowY: 'auto', background: t.panel }}>
          {lines.map(l => {
            ...
            const cellColor = l.pending ? t.textFaint :
                              belowNyq ? '#d97706' :
                              pass ? t.success : t.danger;
            return (
              <div key={l.id} onClick={(e) => toggleSel(l.id, e.nativeEvent)}
                style={{
                  display: 'grid', gridTemplateColumns: cg, gap: 0, padding: '6px 10px', fontSize: 11,
                ...
                <input type="checkbox" checked={sel}
                       onChange={() => {}}
                       onClick={(e) => { e.stopPropagation(); toggleSel(l.id, e.nativeEvent); }}
                       style={{ margin: 0, cursor: 'pointer' }} />
                <div>{l.group}</div>
                <div>{l.element}</div>
                <div style={{ color: usafColorForElt(l.element) }}>{l.direction}</div>
                <div>{(l.m ? l.m.lp_mm : usafLpmm(l.group, l.element)).toFixed(2)}</div>
                <div style={{ color: cellColor, fontWeight: 500 }} title={l.pending ? 'measuring…' : pass ? `Pass (≥${(threshold * 100).toFixed(0)}%)` : 'Fail'}>
                  {l.pending ? '…' : mich != null ? mich.toFixed(3) : 'err'}
                  {ispApplied && l.m && <span style={{ color: t.accent, marginLeft: 2 }}>*</span>}
                </div>
                <div style={{ color: t.textMuted }}>{l.m ? l.m.modulation_fft.toFixed(3) : '—'}</div>
                <div style={{ color: l.m ? sCycColor(l.m.samples_per_cycle, t) : t.textFaint, fontWeight: 500 }}
          ...
          {lines.length === 0 && <div style={{ padding: '18px 10px', fontSize: 11, color: t.textFaint, textAlign: 'center' }}>No lines yet — click-click or drag on the canvas.</div>}
        </div>
      </div>
      <div style={{ display: 'flex', gap: 6, marginTop: 8, flexWrap: 'wrap' }}>
        <Tip title={`Delete ${selectedIds.size} selected`}><Button icon="trash" size="xs" variant="danger" disabled={selectedIds.size === 0} onClick={onDelete}>Delete{selectedIds.size > 1 ? ` (${selectedIds.size})` : ''}</Button></Tip>
        <Tip title="Select all lines (⌘A)"><Button size="xs" onClick={onSelectAll} disabled={totalLines === 0}>Select all</Button></Tip>
        <Tip title="Delete all lines"><Button size="xs" onClick={onClearAll} disabled={totalLines === 0}>Clear all</Button></Tip>
        <div style={{ flex: 1 }} />
        <Tip title="Download measured lines as CSV"><Button size="xs" icon="export" disabled={totalLines === 0} onClick={onCSV}>CSV</Button></Tip>
      </div>
      <div style={{ fontSize: 10, color: t.textFaint, marginTop: 6, fontFamily: 'ui-monospace,Menlo,monospace' }}>
        ⇧-click range · ⌘-click toggle · click header to sort{ispApplied ? ' · * = ISP-applied' : ''}
      </div>
    </Card>
  );
};
...
const michelson5pt = (profile, bars, gaps) => {
  ...
  const bVals = bars.map(val), gVals = gaps.map(val);
  const mB = mean(bVals), mG = mean(gVals);
  const hi = Math.max(mB, mG), lo = Math.min(mB, mG);
  const d = hi + lo;
  return d > 0 ? (hi - lo) / d : 0;
};

const ProfilePreview = ({ line, method, multiCount, threshold, ispApplied, onPointsChange, onReset }) => {
  const t = useTheme();
  ...
  React.useEffect(() => { setDragOverride(null); }, [line?.id]);

  if (!line) {
    return <div style={{ fontSize: 11, color: t.textFaint, padding: '16px 4px', textAlign: 'center' }}>Select a line to see its profile.</div>;
  }
  const m = line.m;
  if (!m) {
    return <div style={{ fontSize: 11, color: t.textFaint, padding: '16px 4px', textAlign: 'center' }}>Measuring…</div>;
  }
  ...
  const onDragStart = (e, kind, which) => {
    ...
    let currentBars = bars.slice(), currentGaps = gaps.slice();
    const run = (ev) => {
  ...
  return (
    <div>
      {multiCount > 1 && <div style={{ fontSize: 10.5, color: t.textMuted, marginBottom: 6 }}>{multiCount} lines selected · showing G{line.group}E{line.element}{line.direction}</div>}
      <div style={{ background: t.panelAlt, border: `1px solid ${t.border}`, borderRadius: 5, padding: 8, marginTop: 4 }}>
        <svg ref={svgRef} viewBox={`0 0 ${W} ${H}`} width="100%" height={H} preserveAspectRatio="none"
             style={{ touchAction: 'none', display: 'block' }}>
          <line x1={0} y1={p10y} x2={W} y2={p10y} stroke={t.textFaint} strokeWidth={0.5} strokeDasharray="3 3" />
          <line x1={0} y1={p90y} x2={W} y2={p90y} stroke={t.textFaint} strokeWidth={0.5} strokeDasharray="3 3" />
          <polyline points={polyPts.map(p => p.join(',')).join(' ')} fill="none"
                    stroke={usafColorForElt(line.element)} strokeWidth={1.6} strokeLinejoin="round" />
          {/* Bar centers — bright fill, dashed vertical guide. */}
          {bars.map((idx, i) => {
            const x = idxToX(idx), y = yAt(idx);
            return (
              <g key={`b${i}`}>
                <line x1={x} y1={0} x2={x} y2={H} stroke="#ffd54f" strokeWidth={0.6} strokeDasharray="2 3" opacity={0.6} />
                <circle cx={x} cy={y} r={5.5} fill="rgba(15,20,30,0.92)" stroke="#ffd54f" strokeWidth={1.6}
                        onMouseDown={(e) => onDragStart(e, 'bar', i)}
                        style={{ cursor: 'ew-resize' }}>
                  <title>{`Bar ${i + 1} · sample ${idx} · ${profile[idx]?.toFixed?.(0) ?? '—'} DN — drag horizontally`}</title>
                </circle>
                <text x={x} y={y - 8} textAnchor="middle" fontSize={8} fill="#ffd54f"
                      fontFamily="ui-monospace,Menlo,monospace" pointerEvents="none">B{i + 1}</text>
              </g>
            );
          })}
          ...
          {gaps.map((idx, i) => {
            ...
            return (
              <g key={`g${i}`}>
                <line x1={x} y1={0} x2={x} y2={H} stroke="#4a9eff" strokeWidth={0.6} strokeDasharray="2 3" opacity={0.55} />
                <circle cx={x} cy={y} r={4.6} fill="#4a9eff" stroke="rgba(15,20,30,0.92)" strokeWidth={1.4}
                        onMouseDown={(e) => onDragStart(e, 'gap', i)}
                        style={{ cursor: 'ew-resize' }}>
                  <title>{`Gap ${i + 1} · sample ${idx} · ${profile[idx]?.toFixed?.(0) ?? '—'} DN — drag horizontally`}</title>
                </circle>
                <text x={x} y={H - 2} textAnchor="middle" fontSize={8} fill="#8ebfff"
                      fontFamily="ui-monospace,Menlo,monospace" pointerEvents="none">G{i + 1}</text>
              </g>
            );
          })}
        </svg>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginTop: 4, fontSize: 9.5, color: t.textFaint, fontFamily: 'ui-monospace,Menlo,monospace' }}>
          ...
          <span>L = {m.line_length_px.toFixed(0)} px · {m.profile_max.toFixed(0)} DN</span>
        </div>
      </div>
      <div style={{ marginTop: 8 }}>
        <div style={{ fontSize: 10, color: t.textMuted, textTransform: 'uppercase', letterSpacing: 0.4, fontWeight: 600, marginBottom: 4, display: 'flex', alignItems: 'center', gap: 6 }}>
          <span>Michelson · {methodLabel}</span>
          {ispApplied && <span title="ISP pipeline live-applied" style={{ color: t.accent, display: 'inline-flex', alignItems: 'center', gap: 3, fontSize: 9.5 }}><Icon name="sparkles" size={9} /> ISP</span>}
          <div style={{ flex: 1 }} />
          {manual && <Button size="xs" onClick={onReset} title="Reset to server auto-detected points">Auto-detect</Button>}
        </div>
        <div style={{ display: 'flex', alignItems: 'baseline', gap: 8 }}>
          <span style={{ fontSize: 24, fontWeight: 600, fontFamily: 'ui-monospace,Menlo,monospace', color: pass ? t.success : t.danger, lineHeight: 1 }}>{primary.toFixed(3)}</span>
          <span style={{ fontSize: 10.5, color: pass ? t.success : t.danger, fontWeight: 500 }}>{pass ? 'PASS' : 'FAIL'}</span>
          <span style={{ fontSize: 10, color: t.textFaint }}>t={(threshold * 100).toFixed(0)}%</span>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 6, marginTop: 10, fontSize: 10.5, color: t.textMuted, fontFamily: 'ui-monospace,Menlo,monospace' }}>
          <div><span style={{ color: t.textFaint }}>5-point</span><div style={{ color: t.accent, fontWeight: 600 }}>{primary5pt.toFixed(3)}</div></div>
          <div><span style={{ color: t.textFaint }}>FFT fund.</span><div style={{ color: t.text }}>{m.modulation_fft.toFixed(3)}</div></div>
          <div><span style={{ color: t.textFaint }}>min/max</span><div style={{ color: t.text }}>{m.modulation_minmax.toFixed(3)}</div></div>
          <div><span style={{ color: t.textFaint }}>lp/mm</span><div style={{ color: t.text }}>{m.lp_mm.toFixed(2)}</div></div>
          <div><span style={{ color: t.textFaint }}>s/cyc</span><div style={{ color: sCycColor(m.samples_per_cycle, t) }}>{m.samples_per_cycle.toFixed(1)}</div></div>
        </div>
        {m.samples_per_cycle < 3 && (
          <div style={{ marginTop: 8, padding: '6px 8px', background: 'rgba(207,34,46,0.08)', border: `1px solid ${t.danger}33`, borderRadius: 4, fontSize: 10.5, color: t.danger, lineHeight: 1.4 }}>
            ⚠ {m.samples_per_cycle.toFixed(1)} samples/cycle — below Nyquist. Values not trustworthy.
          </div>
        )}
      </div>
    </div>
...
const RulerH = ({ t, imgSize, step, ticks, zoom, panPx, cursorImg, leftInset }) => (
  <div style={{ position: 'absolute', top: 0, left: leftInset, right: 0, height: 20, background: t.panelAlt, borderBottom: `1px solid ${t.border}`, pointerEvents: 'none', overflow: 'hidden' }}>
    <div style={{ position: 'relative', width: '100%', height: '100%', transform: `translateX(${panPx}px)` }}>
      <svg width="100%" height="20" preserveAspectRatio="none" style={{ position: 'absolute', inset: 0 }}>
        {ticks.map(v => {
      ...
      {cursorImg != null && (() => {
        ...
        return <div style={{ position: 'absolute', top: 0, left: `${pct * 100}%`, width: 1, height: '100%', background: '#e5484d' }} />;
      })()}
    </div>
  </div>
);
const RulerV = ({ t, imgSize, step, ticks, zoom, panPx, cursorImg, topInset }) => (
  <div style={{ position: 'absolute', top: topInset, left: 0, bottom: 0, width: 26, background: t.panelAlt, borderRight: `1px solid ${t.border}`, pointerEvents: 'none', overflow: 'hidden' }}>
    <div style={{ position: 'relative', width: '100%', height: '100%', transform: `translateY(${panPx}px)` }}>
      <svg width="26" height="100%" preserveAspectRatio="none" style={{ position: 'absolute', inset: 0 }}>
        {ticks.map(v => {
          const pct = ((v / imgSize) - 0.5) * zoom + 0.5;
          if (pct < -0.05 || pct > 1.05) return null;
          const major = (v % (step * 5) === 0) || step >= 100;
          return (
            <g key={v}>
              <line x1={major ? 14 : 20} x2={26} y1={`${pct * 100}%`} y2={`${pct * 100}%`} stroke={t.textFaint} strokeWidth={major ? 0.8 : 0.5} />
              {major && <text x={2} y={`${(pct * 100).toFixed(2)}%`} dy={-2} fontSize={9} fill={t.textMuted} fontFamily="ui-monospace,Menlo,monospace">{v}</text>}
            </g>
          );
        })}
      </svg>
      {cursorImg != null && (() => {
        const pct = ((cursorImg / imgSize) - 0.5) * zoom + 0.5;
        if (pct < 0 || pct > 1) return null;
        return <div style={{ position: 'absolute', left: 0, top: `${pct * 100}%`, height: 1, width: '100%', background: '#e5484d' }} />;
      })()}
    </div>
  </div>
);

...
// ---------------------------------------------------------------------------
const GridTabFrame = ({ caption, n, children, minCardPx = 380,
                        extraControls, storageKey = 'analysis/defaultLayout' }) => {
  const t = useTheme();
...
const PlotlyChart = ({ data, layout, config, style }) => {
  ...
  useEffectA(() => {
    ...
    const merged = {
      paper_bgcolor: t.panel, plot_bgcolor: t.panelAlt,
      font: { family: 'Inter Tight, -apple-system, system-ui, sans-serif',
              size: 12, color: t.text },
      margin: { l: 60, r: 16, t: 28, b: 50 },
      autosize: true,
      ...layout,
      xaxis: { gridcolor: t.border, zerolinecolor: t.border, linecolor: t.borderStrong, tickcolor: t.textMuted, ...(layout?.xaxis || {}) },
      yaxis: { gridcolor: t.border, zerolinecolor: t.border, linecolor: t.borderStrong, tickcolor: t.textMuted, ...(layout?.yaxis || {}) },
...
const COLOR_MAPS = {
  jet: [[0,'#00007f'],[0.125,'#0000ff'],[0.375,'#00ffff'],[0.625,'#ffff00'],[0.875,'#ff0000'],[1,'#7f0000']],
  turbo: [[0,'#30123b'],[0.15,'#4145ab'],[0.3,'#26bce1'],[0.5,'#a6f54b'],[0.7,'#fdbb2a'],[0.85,'#f56b14'],[1,'#7a0403']],
  viridis: [[0,'#440154'],[0.25,'#3b528b'],[0.5,'#21918c'],[0.75,'#5ec962'],[1,'#fde725']],
  magma: [[0,'#000004'],[0.25,'#3b0f70'],[0.5,'#8c2981'],[0.75,'#de4968'],[1,'#fcfdbf']],
  inferno: [[0,'#000004'],[0.25,'#420a68'],[0.5,'#932667'],[0.75,'#dd513a'],[1,'#fcffa4']],
  plasma: [[0,'#0d0887'],[0.25,'#6a00a8'],[0.5,'#b12a90'],[0.75,'#e16462'],[1,'#fca636']],
  cividis: [[0,'#00204c'],[0.25,'#414d6b'],[0.5,'#7d7c78'],[0.75,'#c5b471'],[1,'#fde737']],
  hot:    [[0,'#000000'],[0.33,'#ff0000'],[0.66,'#ffff00'],[1,'#ffffff']],
  cool:   [[0,'#00ffff'],[1,'#ff00ff']],
...
const measurementToRow = (ch, spec, m) => {
  ...
  return {
    channel: ch,
    group: spec.group, element: spec.element, direction: spec.direction,
    p0_x: spec.p0[0], p0_y: spec.p0[1], p1_x: spec.p1[0], p1_y: spec.p1[1],
...
const BgColorPicker = ({ bgColor, setBgColor }) => {
  ...
  const presets = [
    ...
    { id: '#ffffff',       label: 'White',     swatch: '#ffffff',  title: 'Solid white — best for journals/print' },
    { id: '#000000',       label: 'Black',     swatch: '#000000',  title: 'Solid black — best for dark-themed slides' },
    { id: 'transparent',   label: 'Clear',     swatch: 'transparent', title: 'Transparent — exports a PNG with alpha' },
  ];
  ...
  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 6, marginLeft: 6 }}>
      <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
                     letterSpacing: 0.5, textTransform: 'uppercase' }}>Background</span>
      {presets.map(p => {
        const active = bgColor === p.id;
        return (
          <button key={String(p.id)} onClick={() => setBgColor(p.id)} title={p.title}
                  style={{
                    width: 22, height: 22, padding: 0, border: `2px solid ${active ? t.accent : t.border}`,
      ...
      <Tip title="Custom — pick any hex color">
        <input type="color" value={isCustom ? bgColor : (bgColor || '#ffffff')}
               onChange={(e) => setBgColor(e.target.value)}
               style={{ width: 24, height: 24, padding: 0, border: `2px solid ${isCustom ? t.accent : t.border}`,
...
const AnalysisModal = ({ run, onClose, onToast }) => {
  if (!run) return null;
  if (run.mode === 'usaf') return <USAFAnalysisModal run={run} onClose={onClose} onToast={onToast} />;
...
const USAFAnalysisModal = ({ run, onClose, onToast }) => {
  ...
  const exportCSVRows = useCallbackA(() => {
    ...
    if (!rows.length) { onToast?.('Nothing to export', 'warn'); return; }
  ...
  const exportJSONRaw = useCallbackA(() => {
    exportJSON(`mantis-usaf-${Date.now()}.json`, {
      kind: 'mantis-usaf-analysis', version: 2, exportedAt: new Date().toISOString(),
      channels: visibleChannels, specs: keptIdx.map(i => allSpecs[i]),
      threshold,
      measurements: Object.fromEntries(visibleChannels.map(ch => [ch, keptIdx.map(i => measurements[ch]?.[i] ?? null)])),
      per_channel_detection_limit: Object.fromEntries(visibleChannels.map(ch => [ch, perChLim[ch] ?? null])),
    });
  ...
  return (
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,12,18,0.62)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 80, padding: 24 }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 1500, height: '94vh', background: t.panel, border: `1px solid ${t.border}`, borderRadius: 10, boxShadow: t.shadowLg, display: 'flex', flexDirection: 'column', overflow: 'hidden',
        ...
        <div style={{ padding: '14px 18px', borderBottom: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', gap: 12, flexShrink: 0 }}>
          <div style={{ width: 30, height: 30, borderRadius: 6, background: t.accentSoft, color: t.accent, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="run" size={15} /></div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14.5, fontWeight: 600, color: t.text }}>USAF analysis — {keptIdx.length} line{keptIdx.length !== 1 ? 's' : ''} × {visibleChannels.length} channel{visibleChannels.length !== 1 ? 's' : ''}</div>
            <div style={{ fontSize: 11, color: t.textFaint, fontFamily: 'ui-monospace,Menlo,monospace', marginTop: 1 }}>server detection limit @ {(serverThreshold * 100).toFixed(0)}% · {Object.entries(perChLim).map(([c, v]) => `${c}: ${v != null ? v.toFixed(2) + ' lp/mm' : '—'}`).join('  ·  ')}</div>
          </div>
          <PlotStylePanel open={false} onToggle={() => setStyleOpen(s => !s)} />
          <Button icon="image"    onClick={exportPNG}    title="Capture the active tab as a PNG image">PNG</Button>
          <Button icon="download" onClick={exportCSVRows} title="Flat per-(channel × line) CSV">CSV</Button>
          <Button icon="download" onClick={exportJSONRaw} title="Round-trippable raw analysis JSON">JSON</Button>
        ...
        {/* Filter bar */}
        <div style={{ padding: '10px 18px', borderBottom: `1px solid ${t.border}`, background: t.panelAlt, display: 'flex', gap: 14, alignItems: 'center', flexWrap: 'wrap', flexShrink: 0 }}>
          <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase' }}>Channels</span>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {allChannels.map(c => (
              <ChannelChip key={c} id={c.includes('-') ? c : `HG-${c}`} multi
                selected={chans.includes(c)}
                onToggle={() => setChans(prev => prev.includes(c) ? prev.filter(x => x !== c) : [...prev, c])}
                size="sm" />
            ))}
            <Button size="xs" onClick={() => setChans(allChannels)}>All</Button>
            <Button size="xs" onClick={() => setChans([])}>None</Button>
          </div>
          {allChannels.some(c => c.startsWith('HG-')) && <>
            <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>Gain</span>
            <Segmented value={gainFilter} onChange={setGainFilter} options={[{ value: 'all', label: 'All' }, { value: 'HG', label: 'HG' }, { value: 'LG', label: 'LG' }]} />
          </>}
          <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>Direction</span>
          <Segmented value={dirFilter} onChange={setDirFilter} options={[{ value: 'all', label: 'All' }, { value: 'H', label: 'H' }, { value: 'V', label: 'V' }]} />
          <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>Threshold</span>
          <input type="range" min={0.05} max={0.95} step={0.01} value={threshold}
                 onChange={(e) => setThreshold(parseFloat(e.target.value))}
                 style={{ width: 160 }} className="rgbnir-slider" />
          <span style={{ fontSize: 11, color: t.text, fontFamily: 'ui-monospace,Menlo,monospace', minWidth: 42 }}>{(threshold * 100).toFixed(0)}%</span>
          <BgColorPicker bgColor={bgColor} setBgColor={setBgColor} />
        </div>

        {/* Tab bar */}
        <div style={{ display: 'flex', borderBottom: `1px solid ${t.border}`, background: t.panelAlt, padding: '0 8px', gap: 2, overflowX: 'auto', flexShrink: 0 }}>
          {tabs.map(x => (
            <button key={x.key} onClick={() => setTab(x.key)} style={{
              padding: '10px 14px', fontSize: 12.5, fontWeight: tab === x.key ? 600 : 450,
        ...
        <div ref={tabBodyRef} style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: 16,
          ...
          {tab === 'mtf'      && <MTFCurvesTab        channels={visibleChannels} specs={allSpecs} keptIdx={keptIdx} measurements={measurements} threshold={threshold} perChLim={perChLim} />}
          {tab === 'profiles' && <ProfileGalleryTab   channels={visibleChannels} specs={allSpecs} keptIdx={keptIdx} measurements={measurements} threshold={threshold} />}
          {tab === 'table'    && <SummaryTableTab     channels={visibleChannels} specs={allSpecs} keptIdx={keptIdx} measurements={measurements} threshold={threshold} />}
          {tab === 'heatmap'  && <DetectionHeatmapTab channels={visibleChannels} specs={allSpecs} measurements={measurements} threshold={threshold} dirFilter={dirFilter} />}
          {tab === 'group'    && <GroupSweepTab       channels={visibleChannels} specs={allSpecs} keptIdx={keptIdx} measurements={measurements} threshold={threshold} />}
          {tab === 'fft'      && <FFTMTFTab           channels={visibleChannels} specs={allSpecs} keptIdx={keptIdx} measurements={measurements} threshold={threshold} />}
        </div>
      </div>
    </div>
...
const MTFCurvesTab = ({ channels, specs, keptIdx, measurements, threshold, perChLim }) => {
  ...
  const xRange = useMemoA(() => {
    const lps = [];
    for (const ch of channels) for (const i of keptIdx) {
      const m = measurements[ch]?.[i]; if (m?.lp_mm) lps.push(m.lp_mm);
    }
  ...
  if (!channels.length) {
    return <div style={{ color: t.textFaint, textAlign: 'center', paddingTop: 40 }}>No channels match the current filter.</div>;
  }
  return (
    <div>
      {/* Per-tab toolbar — marked [data-no-export] so the PNG capture skips it. */}
      <div data-no-export
           style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12, padding: '8px 10px', background: t.panel, border: `1px solid ${t.border}`, borderRadius: 6, flexWrap: 'wrap' }}>
        <span style={{ fontSize: 10.5, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase' }}>Style</span>
        <Segmented value={style} onChange={setStyle} options={Object.keys(MTF_STYLES).map(k => ({ value: k, label: MTF_STYLES[k].desc }))} />
        <span style={{ fontSize: 10.5, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>H stroke</span>
        <Segmented value={dashH} onChange={setDashH} options={Object.keys(MTF_DASHES).map(k => ({ value: k, label: k }))} />
        <span style={{ fontSize: 10.5, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>V stroke</span>
        <Segmented value={dashV} onChange={setDashV} options={Object.keys(MTF_DASHES).map(k => ({ value: k, label: k }))} />
        <span style={{ fontSize: 10.5, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>H marker</span>
        <Segmented value={markerH} onChange={setMarkerH} options={Object.keys(MTF_MARKERS).map(k => ({ value: k, label: MTF_MARKERS[k].split(' ')[0] }))} />
        <span style={{ fontSize: 10.5, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>V marker</span>
        <Segmented value={markerV} onChange={setMarkerV} options={Object.keys(MTF_MARKERS).map(k => ({ value: k, label: MTF_MARKERS[k].split(' ')[0] }))} />
        <span style={{ fontSize: 10.5, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>X-axis</span>
        <Segmented value={logX ? 'log' : 'lin'} onChange={(v) => setLogX(v === 'log')} options={[{ value: 'log', label: 'log' }, { value: 'lin', label: 'linear' }]} />
        <span style={{ flex: 1 }} />
      ...
      <div style={{ display: 'grid', gap: 14,
        ...
        {channels.map(ch => (
          <MiniMTFChart key={ch} channel={ch} specs={specs} keptIdx={keptIdx}
                        ...
                        xRange={xRange} drawStyle={MTF_STYLES[style]}
                        dashPatternH={MTF_DASHES[dashH]} dashPatternV={MTF_DASHES[dashV]}
                        markerH={markerH} markerV={markerV} logX={logX} />
        ))}
      </div>
    </div>
  );
...
const MarkerShape = ({ shape, x, y, color, m, size = 7 }) => {
  const r = size / 2;
  const tooltip = `G${m.spec?.group ?? ''}E${m.spec?.element ?? ''} · ${m.lp_mm.toFixed(2)} lp/mm · M=${m.modulation_5pt.toFixed(3)}`;
  const stroke = '#fff', sw = 0.9;
  switch (shape) {
    case 'square':
      return <rect x={x - r} y={y - r} width={size} height={size} fill={color} stroke={stroke} strokeWidth={sw}><title>{tooltip}</title></rect>;
    case 'triangle':
      return <polygon points={`${x},${y - r * 1.15} ${x - r},${y + r * 0.85} ${x + r},${y + r * 0.85}`} fill={color} stroke={stroke} strokeWidth={sw}><title>{tooltip}</title></polygon>;
    case 'diamond':
      return <polygon points={`${x},${y - r * 1.1} ${x + r * 1.1},${y} ${x},${y + r * 1.1} ${x - r * 1.1},${y}`} fill={color} stroke={stroke} strokeWidth={sw}><title>{tooltip}</title></polygon>;
    case 'cross':
      return (
        <g>
          <line x1={x - r} y1={y - r} x2={x + r} y2={y + r} stroke={color} strokeWidth={scaled(2, style)} />
          <line x1={x - r} y1={y + r} x2={x + r} y2={y - r} stroke={color} strokeWidth={scaled(2, style)} />
          <title>{tooltip}</title>
        </g>
      );
    case 'plus':
      return (
        <g>
          <line x1={x - r * 1.1} y1={y} x2={x + r * 1.1} y2={y} stroke={color} strokeWidth={scaled(2, style)} />
          <line x1={x} y1={y - r * 1.1} x2={x} y2={y + r * 1.1} stroke={color} strokeWidth={scaled(2, style)} />
          <title>{tooltip}</title>
        </g>
      );
    case 'circle':
    default:
      return <circle cx={x} cy={y} r={r} fill={color} stroke={stroke} strokeWidth={sw}><title>{tooltip}</title></circle>;
  }
};
...
const MiniMTFChart = ({ channel, specs, keptIdx, measurements, threshold, detectionLimit, xRange,
                        drawStyle = MTF_STYLES['lines+markers'],
                        dashPatternH = '', dashPatternV = '6 4',
                        markerH = 'circle', markerV = 'square', logX = true }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const W = 360, H = 220, PAD_L = 44, PAD_R = 14, PAD_T = 14, PAD_B = 36;
  ...
  const xTicks = (logX
    ...
    : [...new Set([xRange[0], xRange[0] + (xRange[1] - xRange[0]) / 4,
                   xRange[0] + (xRange[1] - xRange[0]) / 2,
                   xRange[0] + 3 * (xRange[1] - xRange[0]) / 4, xRange[1]].map(v => Math.round(v * 100) / 100))]
  ...
  return (
    <div style={{ ...cardChromeFor(style, t) }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        ...
        <span style={{ flex: 1 }} />
        <span style={{ fontSize: 10.5, color: t.textMuted, fontFamily: 'ui-monospace,Menlo,monospace' }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block', background: 'transparent', borderRadius: 4 }}>
        ...
        {series.map(({ dir, pts }) => pts.length > 0 && (
          <g key={dir}>
            {drawStyle.line && (
              <polyline
            ...
            {drawStyle.marker &&pts.map((p, i) => {
              const shape = dir === 'H' ? markerH : markerV;
              return <MarkerShape key={i} shape={shape} x={p.x} y={p.y} color={color} m={p.m} />;
            })}
          </g>
        ))}
        {/* Axis labels */}
...
const ProfileGalleryTab = ({ channels, specs, keptIdx, measurements, threshold }) => {
  ...
  if (!keptIdx.length || !channels.length) {
    return <div style={{ color: t.textFaint, textAlign: 'center', paddingTop: 40 }}>No lines / channels match the current filter.</div>;
  }
  // Flatten to (line × channel) cards.
  const cards = [];
  for (const i of keptIdx) for (const ch of channels) {
  ...
  return (
    <GridTabFrame
      ...
      n={cards.length} minCardPx={220}
      storageKey="analysis/usaf/profilesLayout">
      {cards.map(({ i, ch, spec, m }) => <ProfileCard key={`${ch}_${i}`} ch={ch} spec={spec} m={m} threshold={threshold} />)}
    </GridTabFrame>
  );
};
...
const ProfileCard = ({ ch, spec, m, threshold }) => {
  ...
  if (!m) {
    return (
      <div style={{ background: t.panel, border: `1px solid ${t.border}`, borderRadius: 6, padding: 8, opacity: 0.5 }}>
        <div style={{ fontSize: 11, color: t.textFaint }}>{ch} · G{spec.group}E{spec.element}{spec.direction} — no data</div>
      </div>
    );
  }
  ...
  const yMin = m.profile_min - vRaw * 0.10;
  const yMax = m.profile_max + vRaw * 0.10;
  const range = yMax - yMin || 1;
  const pts = profile.map((v, idx) => `${(idx / (N - 1 || 1)) * W},${(1 - (v - yMin) / range) * H}`).join(' ');
  // Bar/gap markers from server detection.
  const marks = [...(m.bar_indices || []).map(j => ({ j, kind: 'bar' })),
                 ...(m.gap_indices || []).map(j => ({ j, kind: 'gap' }))];
  return (
    <div style={{ background: t.panel, border: `2px solid ${borderColor}`, borderRadius: 6, padding: 8 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 5, marginBottom: 4 }}>
        <span style={{ width: 8, height: 8, borderRadius: '50%', background: color }} />
        <span style={{ fontSize: 11, color: t.text, fontFamily: 'ui-monospace,Menlo,monospace', flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
          {ch} · G{spec.group}E{spec.element}{spec.direction}
        </span>
      </div>
      ...
      <div style={{ fontSize: 10, color: t.textMuted, marginTop: 3, fontFamily: 'ui-monospace,Menlo,monospace', display: 'flex', justifyContent: 'space-between' }}>
        ...
        <span style={{ color: belowNyq ? '#d97706' : pass ? t.success : t.danger, fontWeight: 600 }}>
...
const SummaryTableTab = ({ channels, specs, keptIdx, measurements, threshold }) => {
  ...
  const sorted = useMemoA(() => {
    ...
    const key = ({
      order:   r => r.order,
      channel: r => r.ch,
      g: r => r.spec.group, e: r => r.spec.element, d: r => r.spec.direction,
      lpmm: r => r.m?.lp_mm ?? -1,
    ...
    c.sort((a, b) => { const va = key(a), vb = key(b); return (va > vb ? 1 : va < vb ? -1 : 0) * (sortDir === 'asc' ? 1 : -1); });
    return c;
  }, [rows, sortCol, sortDir]);
  const setSort = (col) => sortCol === col ? setSortDir(d => d === 'asc' ? 'desc' : 'asc') : (setSortCol(col), setSortDir('asc'));
  ...
  const cols = [
    ...
    { key: 'g', label: 'G' }, { key: 'e', label: 'E' }, { key: 'd', label: 'D' },
  ...
  return (
    <div style={{ background: t.panel, border: `1px solid ${t.border}`, borderRadius: 8, overflow: 'hidden' }}>
      <div style={{ padding: '10px 14px', borderBottom: `1px solid ${t.border}`, fontSize: 12, color: t.textMuted, display: 'flex', alignItems: 'center', gap: 14 }}>
        <span>{sorted.length} rows · {channels.length} channel{channels.length !== 1 ? 's' : ''} × {keptIdx.length} line{keptIdx.length !== 1 ? 's' : ''}</span>
        <span style={{ flex: 1 }} />
        <span style={{ fontSize: 10.5 }}>click a header to sort</span>
      </div>
      <div style={{ maxHeight: 'calc(94vh - 320px)', overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontFamily: 'ui-monospace,Menlo,monospace', fontSize: 12 }}>
          <thead style={{ position: 'sticky', top: 0, background: t.panelAlt, zIndex: 1 }}>
            <tr>
              {cols.map(c => (
                <th key={c.key} onClick={() => c.key !== 'pass' && setSort(c.key)}
                    style={{
                      textAlign: c.align || 'right',
                      padding: '8px 12px', fontSize: 10.5, textTransform: 'uppercase', letterSpacing: 0.5, fontWeight: 600,
                      color: t.textMuted, borderBottom: `1px solid ${t.border}`,
                      cursor: c.key !== 'pass' ? 'pointer' : 'default', userSelect: 'none', whiteSpace: 'nowrap',
                    }}>
            ...
            {sorted.map((r, idx) => {
              ...
              return (
                <tr key={idx} style={{ background: rowBg, borderBottom: `1px solid ${t.border}` }}>
                  <td style={{ padding: '7px 12px', color: chColor, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 7, height: 7, borderRadius: '50%', background: chColor }} />{r.ch}
                  </td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.text }}>{r.spec.group}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.text }}>{r.spec.element}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.text }}>{r.spec.direction}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.text }}>{m ? m.lp_mm.toFixed(2) : '—'}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: mColor, fontWeight: 600 }}>{m ? m.modulation_5pt.toFixed(3) : '—'}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>{m ? m.modulation_fft.toFixed(3) : '—'}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>{m ? m.modulation_minmax.toFixed(3) : '—'}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: below ? '#d97706' : t.textMuted }}>{m ? m.samples_per_cycle.toFixed(1) : '—'}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: pass ? t.success : t.danger, fontWeight: 600 }}>{m ? (pass ? '✓' : '✗') : '—'}</td>
                </tr>
              );
            })}
            {sorted.length === 0 && (<tr><td colSpan={cols.length} style={{ padding: 40, textAlign: 'center', color: t.textFaint }}>No rows.</td></tr>)}
          </tbody>
        </table>
      </div>
    </div>
  ...
  const matrix = useMemoA(() => {
    ...
    for (const ch of channels) {
      ...
      specs.forEach((s, i) => {
        ...
        cell[k].n += 1;
        cell[k].samples_per_cycle = Math.min(cell[k].samples_per_cycle || Infinity, m.samples_per_cycle);
      });
      out[ch] = cell;
    }
    return out;
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
        <span style={{ fontSize: 10.5, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase' }}>Colormap</span>
        <select value={cmap} onChange={(e) => setCmap(e.target.value)}
                style={{ background: t.chipBg, color: t.text, border: `1px solid ${t.chipBorder}`, borderRadius: 4, padding: '4px 8px', fontSize: 11.5, fontFamily: 'inherit' }}>
          {Object.keys(COLOR_MAPS).map(c => <option key={c} value={c}>{c}</option>)}
        </select>
        <span style={{ flex: 1 }} />
        <span style={{ fontSize: 11, color: t.textMuted, fontFamily: 'ui-monospace,Menlo,monospace' }}>
          rows = USAF group · cols = element 1–6 · cell value = mean Mich (5-pt) over all picked H/V at that G/E
        </span>
      </div>
      <div style={{ display: 'grid', gridTemplateColumns: `repeat(${channels.length}, minmax(220px, 1fr)) 56px`, gap: 14 }}>
        {channels.map(ch => (
          <HeatmapPanel key={ch} channel={ch} cells={matrix[ch] || {}} cmap={cmap} threshold={threshold} />
        ))}
...
const HeatmapPanel = ({ channel, cells, cmap, threshold }) => {
  ...
  return (
    <div style={{ ...cardChromeFor(style, t) }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '24px repeat(6, 1fr)', gap: 1, alignItems: 'stretch' }}>
        <div />
        {elements.map(e => <div key={e} style={{ fontSize: 9.5, color: t.textFaint, textAlign: 'center', padding: 2 }}>E{e}</div>)}
        {groups.map(g => (
          <React.Fragment key={g}>
            <div style={{ fontSize: 9.5, color: t.textFaint, textAlign: 'center', padding: 2, alignSelf: 'center' }}>G{g}</div>
            {elements.map(e => {
              const cell = cells[`${g}_${e}`];
              if (!cell) {
                return <div key={e} style={{ aspectRatio: '1/1', background: t.panelAlt, border: `1px dashed ${t.border}`, display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 8, color: t.textFaint }}>—</div>;
              }
              ...
              return (
                <div key={e} title={`${channel} · G${g}E${e} · M=${v.toFixed(3)} · ${isPass ? 'PASS' : 'FAIL'} · n=${cell.n} pick${cell.n !== 1 ? 's' : ''}`}
                     style={{ aspectRatio: '1/1', background: bg, color: fg, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center',
...
const Colorbar = ({ cmap, threshold, t }) => {
  ...
  return (
    <div style={{ position: 'relative', minHeight: 200 }}>
      <div style={{ position: 'absolute', top: 22, bottom: 16, left: 0, width: 16, background: `linear-gradient(to top, ${stops.map(([u,c]) => `${c} ${u*100}%`).join(', ')})`, borderRadius: 3, border: `1px solid ${t.border}` }} />
      <div style={{ position: 'absolute', top: 4, left: 18, fontSize: 9, color: t.textMuted, fontFamily: 'ui-monospace,Menlo,monospace' }}>1.00</div>
      <div style={{ position: 'absolute', bottom: 0, left: 18, fontSize: 9, color: t.textMuted, fontFamily: 'ui-monospace,Menlo,monospace' }}>0.00</div>
      <div style={{ position: 'absolute', top: `calc(22px + ${(1 - threshold) * 100}% - 22px * ${(1 - threshold)})`, left: 0, width: 22, height: 1, background: t.text }} />
      <div style={{ position: 'absolute', top: `calc(22px + ${(1 - threshold) * 100}% - 22px * ${(1 - threshold)} - 6px)`, left: 18, fontSize: 9, color: t.text, fontWeight: 600, fontFamily: 'ui-monospace,Menlo,monospace' }}>{(threshold * 100).toFixed(0)}%</div>
    </div>
  );
};
...
const GroupSweepTab = ({ channels, specs, keptIdx, measurements, threshold }) => {
  ...
  return (
    <GridTabFrame
      ...
      n={groups.length} minCardPx={260}
      storageKey="analysis/usaf/groupSweepLayout">
      {groups.map(g => (
        <GroupMiniChart key={g} group={g} channels={channels} specs={specs} keptIdx={keptIdx}
                        measurements={measurements} threshold={threshold} />
      ))}
    </GridTabFrame>
  );
...
const GroupMiniChart = ({ group, channels, specs, keptIdx, measurements, threshold }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const W = 260, H = 170, PAD_L = 36, PAD_R = 12, PAD_T = 14, PAD_B = 30;
  const elements = [1, 2, 3, 4, 5, 6];
...
const FFTMTFTab = ({ channels, specs, keptIdx, measurements, threshold }) => {
  ...
  if (!channels.length || !keptIdx.length) {
    return <div style={{ color: t.textFaint, textAlign: 'center', paddingTop: 40 }}>No data to display.</div>;
  }
  return (
    <div>
      <div data-no-export
           style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 14, padding: '8px 10px', background: t.panel, border: `1px solid ${t.border}`, borderRadius: 6 }}>
        <span style={{ fontSize: 10.5, color: t.textMuted, fontWeight: 600, letterSpacing: 0.5, textTransform: 'uppercase' }}>View</span>
        ...
        <span style={{ flex: 1 }} />
        <span style={{ fontSize: 10.5, color: t.textFaint, fontFamily: 'ui-monospace,Menlo,monospace' }}>
          CTF = raw 5-point Michelson · Coltman = (π/4)·CTF (sine-wave MTF approximation) · FFT = per-line spectra
        </span>
      </div>
      {view === 'fft'
        ? <FFTSpectraGrid channels={channels} specs={specs} keptIdx={keptIdx} measurements={measurements} />
        : <FFTMTFOverlay  channels={channels} specs={specs} keptIdx={keptIdx} measurements={measurements} threshold={threshold} coltman={view === 'coltman'} />}
    </div>
  );
};
  ...
  for (const ch of channels) {
    for (const dir of ['H', 'V']) {
      ...
      data.push({
        ...
        marker: { color, size: 8, symbol: dir === 'V' ? 'square' : 'circle', line: { color: '#fff', width: 1 } },
        hovertemplate: '%{x:.2f} lp/mm<br>%{y:.3f}<extra>%{fullData.name}</extra>',
      });
    }
  }
  return (
    <div style={{ height: 'calc(94vh - 320px)', minHeight: 480, background: t.panel, border: `1px solid ${t.border}`, borderRadius: 8, padding: 8 }}>
      <PlotlyChart
        data={data}
        layout={{
          xaxis: { type: 'log', title: { text: 'spatial frequency (lp/mm @ chart)' } },
          yaxis: { title: { text: coltman ? 'MTF (Coltman, sine-wave approx.)' : 'CTF (5-point Michelson)' }, range: [0, coltman ? 0.85 : 1.05] },
          shapes: [{ type: 'line', xref: 'paper', yref: 'y', x0: 0, x1: 1, y0: threshold * (coltman ? Math.PI / 4 : 1), y1: threshold * (coltman ? Math.PI / 4 : 1), line: { color: t.warn, width: 1, dash: 'dash' } }],
...
const FFTSpectraGrid = ({ channels, specs, keptIdx, measurements }) => {
  ...
  const fftMag = (profile) => {
    const N = profile.length; if (N < 4) return { f: [], m: [] };
    const mean = profile.reduce((a, b) => a + b, 0) / N;
    const p = profile.map(v => v - mean);
    const K = Math.min(Math.floor(N / 2) + 1, 256);
    const f = new Array(K), m = new Array(K);
    let mx = 0;
    for (let k = 0; k < K; k++) {
      let re = 0, im = 0; const w = -2 * Math.PI * k / N;
      for (let n = 0; n < N; n++) { re += p[n] * Math.cos(w * n); im += p[n] * Math.sin(w * n); }
      const mag = Math.sqrt(re * re + im * im);
      f[k] = k / N; m[k] = mag; if (mag > mx) mx = mag;
    }
    if (mx > 0) for (let k = 0; k < K; k++) m[k] /= mx;
    return { f, m };
  };
  return (
    <GridTabFrame n={keptIdx.length} minCardPx={360}
                  storageKey="analysis/usaf/fftSpectraLayout">
      {keptIdx.map(i => {
        const spec = specs[i];
        const W = 340, H = 180, PAD_L = 36, PAD_R = 10, PAD_T = 12, PAD_B = 30;
...
const FPNAnalysisModal = ({ run, onClose, onToast }) => {
  ...
  const exportCSVRows = useCallbackA(() => {
    ...
    for (const ch of visibleChannels) {
      ...
      for (const i of visibleRoiIdx) {
        ...
        rows.push({
          channel: ch, roi_idx: i, roi_label: run.rois?.[i]?.label ?? '',
          y0: roi[0], x0: roi[1], y1: roi[2], x1: roi[3],
  ...
  const exportJSONRaw = useCallbackA(() => {
    exportJSON(`mantis-fpn-${Date.now()}.json`, {
      kind: 'mantis-fpn-analysis', version: 1, exportedAt: new Date().toISOString(),
      channels: visibleChannels, rois: allRois, settings,
      measurements: Object.fromEntries(visibleChannels.map(ch => [ch, visibleRoiIdx.map(i => measurements[ch]?.[i] ?? null)])),
    });
    onToast?.('Exported analysis JSON', 'success');
  ...
  return (
    <PlotStyleCtx.Provider value={plotStyleState}>
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,12,18,0.62)',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  zIndex: 80, padding: 24 }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()}
           style={{ width: '100%', maxWidth: 1500, height: '94vh',
        ...
        <div style={{ padding: '14px 18px', borderBottom: `1px solid ${t.border}`,
          ...
          <div style={{ width: 30, height: 30, borderRadius: 6, background: t.accentSoft, color: t.accent,
                        display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
            <Icon name="run" size={15} />
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14.5, fontWeight: 600, color: t.text }}>
              FPN analysis — {allRois.length} ROI{allRois.length !== 1 ? 's' : ''} × {visibleChannels.length} channel{visibleChannels.length !== 1 ? 's' : ''}
            </div>
            <div style={{ fontSize: 11, color: t.textFaint,
                          fontFamily: 'ui-monospace,Menlo,monospace', marginTop: 1 }}>
              drift={settings.drift_order || 'none'} · hot |z|={settings.hot_sigma ?? 4} · outlier cuts {(settings.lo_pct || 0).toFixed(1)} / {(settings.hi_pct || 0).toFixed(1)}%
              {settings.median_size ? ` · median ${settings.median_size}×${settings.median_size}` : ''}
              {settings.gaussian_sigma > 0.05 ? ` · gauss σ=${settings.gaussian_sigma.toFixed(2)}` : ''}
              {settings.bilateral ? ' · bilateral' : ''}
            </div>
          </div>
          <PlotStylePanel open={false} onToggle={() => setStyleOpen(s => !s)} />
          <Button icon="image"    onClick={exportPNG}    title="Capture the active tab as a PNG">PNG</Button>
          <Button icon="download" onClick={exportCSVRows} title="Flat per-(channel × ROI) CSV">CSV</Button>
          <Button icon="download" onClick={exportJSONRaw} title="Round-trippable raw analysis JSON">JSON</Button>
          <Button variant="subtle" icon="close" onClick={onClose} size="sm" title="Close (Esc)" />
        </div>
        {styleOpen && <PlotStylePanel open={true} onToggle={() => setStyleOpen(false)} />}
        ...
        <div style={{ padding: '10px 18px', borderBottom: `1px solid ${t.border}`,
                      background: t.panelAlt, display: 'flex', gap: 14, alignItems: 'center',
                      flexWrap: 'wrap', flexShrink: 0 }}>
          <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
                         letterSpacing: 0.5, textTransform: 'uppercase' }}>Channels</span>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 4 }}>
            {allChannels.map(c => (
              <ChannelChip key={c} id={c.includes('-') ? c : `HG-${c}`} multi
                selected={chans.includes(c)}
          ...
          {allChannels.some(c => c.startsWith('HG-')) && <>
            <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
                           letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>Gain</span>
            <Segmented value={gainFilter} onChange={setGainFilter} options={[
              { value: 'all', label: 'All' }, { value: 'HG', label: 'HG' }, { value: 'LG', label: 'LG' },
            ]} />
          </>}
          <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
                         letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>ROI</span>
          <Segmented value={roiIdxFilter} onChange={setRoiIdxFilter} options={[
            { value: 'all', label: 'All' },
          ...
          <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
                         letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>Unit</span>
        ...
        <div style={{ display: 'flex', borderBottom: `1px solid ${t.border}`,
                      background: t.panelAlt, padding: '0 8px', gap: 2,
        ...
        <div ref={tabBodyRef} style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: 16,
                                        ...
                                        backgroundSize: bgColor === 'transparent' ? '16px 16px' : undefined,
                                        backgroundPosition: bgColor === 'transparent' ? '0 0, 0 8px, 8px -8px, -8px 0px' : undefined }}>
          {tab === 'summary' && <FPNSummaryTab channels={visibleChannels} measurements={measurements}
                                                rois={allRois} visibleRoiIdx={visibleRoiIdx}
                                                roiLabel={roiLabel} setRoiLabel={setRoiLabel}
                                                unit={unit} fullDR={FULL_DR} />}
          {tab === 'hist' && <FPNHistogramsTab channels={visibleChannels} measurements={measurements}
                                                visibleRoiIdx={visibleRoiIdx} roiLabel={roiLabel}
                                                unit={unit} fullDR={FULL_DR} />}
          {tab === 'profiles' && <FPNProfilesTab channels={visibleChannels} measurements={measurements}
...
// ---------------------------------------------------------------------------
const FPNSummaryTab = ({ channels, measurements, rois, visibleRoiIdx, roiLabel,
                          setRoiLabel, unit = 'DN', fullDR = 65535 }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const [sortCol, setSortCol] = useStateA('order');
  const [sortDir, setSortDir] = useStateA('asc');
  ...
  const fmtDN  = (v, decimals = 3) => v == null ? '—'
                  : unit === 'pctDR' ? `${(v / fullDR * 100).toFixed(decimals + 1)}`
                                     : v.toFixed(decimals);
  ...
  const rowsAll = useMemoA(() => {
    const out = [];
    for (const ch of channels) for (const i of visibleRoiIdx) {
      const m = measurements[ch]?.[i];
      out.push({ order: out.length, ch, i, m,
                 label: roiLabel(i), roi: rois[i] });
    }
    return out;
  ...
  const sorted = useMemoA(() => {
    const c = [...rowsAll];
    const key = ({
      order: r => r.order, channel: r => r.ch, label: r => r.label,
    ...
    c.sort((a, b) => { const va = key(a), vb = key(b);
      return (va > vb ? 1 : va < vb ? -1 : 0) * (sortDir === 'asc' ? 1 : -1); });
    return c;
  }, [rowsAll, sortCol, sortDir]);
  const setSort = (col) => sortCol === col
    ...
    <div style={{ background: t.panel, border: `1px solid ${t.border}`,
      ...
      <div data-no-export
           style={{ padding: '10px 14px', borderBottom: `1px solid ${t.border}`,
                    fontSize: 12, color: t.textMuted, display: 'flex',
                    alignItems: 'center', gap: 14, position: 'relative' }}>
        <span>{sorted.length} rows · {channels.length} channels × {visibleRoiIdx.length} ROIs</span>
        <span style={{ flex: 1 }} />
        <span style={{ fontSize: 10.5 }}>double-click a ROI cell to rename · click a header to sort</span>
        <Tip title="Show / hide columns">
          <button onClick={() => setShowColMenu(v => !v)}
                  style={{ background: t.chipBg, color: t.text, border: `1px solid ${t.border}`,
        ...
        {showColMenu && (
          <div style={{ position: 'absolute', top: '100%', right: 14, zIndex: 5,
                        background: t.panel, border: `1px solid ${t.borderStrong}`,
                        borderRadius: 6, padding: 8, boxShadow: t.shadowLg,
              ...
              <label key={c.key}
                ...
                <input type="checkbox" checked={!hiddenCols[c.key]}
                       onChange={(e) => setHiddenCols(prev =>
                         ({ ...prev, [c.key]: !e.target.checked }))}
                       style={{ margin: 0, cursor: 'pointer' }} />
                <span style={{ fontFamily: 'ui-monospace,Menlo,monospace', fontSize: 11 }}>{c.label}</span>
              </label>
            ))}
            <div style={{ display: 'flex', gap: 4, marginTop: 6, paddingTop: 6,
                          borderTop: `1px solid ${t.border}` }}>
              <Button size="xs" onClick={() => setHiddenCols({})}>All</Button>
              <Button size="xs" onClick={() => setHiddenCols(
                Object.fromEntries(allCols.map(c => [c.key, true])))}>None</Button>
              <span style={{ flex: 1 }} />
              <Button size="xs" onClick={() => setShowColMenu(false)}>Close</Button>
            </div>
          </div>
        )}
      </div>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse',
            ...
            <tr>
              {cols.map(c => (
                <th key={c.key} onClick={() => setSort(c.key)}
                    style={{ textAlign: c.align || 'right', padding: '8px 12px',
                             fontSize: 10.5, textTransform: 'uppercase',
                             letterSpacing: 0.5, fontWeight: 600, color: t.textMuted,
          ...
          <tbody>
            {sorted.map((r, idx) => {
              ...
              const cellByKey = {
                channel: (
                  <td style={{ padding: '7px 12px', color: chColor, fontWeight: 600,
                               display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 7, height: 7, borderRadius: '50%', background: chColor }} />{r.ch}
                  </td>
                ),
                label: (
                  <td style={{ padding: '7px 12px', color: t.text }}
                    ...
                    {editingI === r.i ? (
                      <input autoFocus defaultValue={r.label}
                        ...
                        onBlur={(e) => { setRoiLabel?.(r.i, e.target.value.trim() || `ROI-${r.i + 1}`);
                                          setEditingI(null); }}
                        style={{ width: '100%', background: t.inputBg, color: t.text,
                                 border: `1px solid ${t.accent}`, borderRadius: 3,
                                 fontSize: 12, fontFamily: 'inherit', padding: '1px 4px' }} />
                    ) : (
                      <span onDoubleClick={(e) => { e.stopPropagation(); setEditingI(r.i); }}
                ...
                px: <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>{px || '—'}</td>,
                mean: <td style={{ padding: '7px 12px', textAlign: 'right', color: t.text }}>
                        {fmtDN(m?.mean_signal, 1)}
                      </td>,
                dsnu: <td style={{ padding: '7px 12px', textAlign: 'right', color: t.text }}>
                        {fmtDN(m?.dsnu_dn)}
                      </td>,
                prnu: <td style={{ padding: '7px 12px', textAlign: 'right', color: prnuColor, fontWeight: 600 }}>
                        {m ? m.prnu_pct.toFixed(3) : '—'}
                      </td>,
                row:  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>{fmtDN(m?.row_noise_dn)}</td>,
                col:  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>{fmtDN(m?.col_noise_dn)}</td>,
                res:  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>{fmtDN(m?.residual_pixel_noise_dn)}</td>,
                rowOnly: <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>{fmtDN(m?.dsnu_row_only_dn)}</td>,
                colOnly: <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>{fmtDN(m?.dsnu_col_only_dn)}</td>,
                rowFreq: <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>
                           {m?.row_peak_freq != null ? m.row_peak_freq.toFixed(4) : '—'}
                         </td>,
                colFreq: <td style={{ padding: '7px 12px', textAlign: 'right', color: t.textMuted }}>
                           {m?.col_peak_freq != null ? m.col_peak_freq.toFixed(4) : '—'}
                         </td>,
                hot:  <td style={{ padding: '7px 12px', textAlign: 'right',
                                    color: (m?.hot_pixel_count || 0) > 0 ? t.warn : t.textMuted }}>
                        {m?.hot_pixel_count ?? '—'}
                      </td>,
                cold: <td style={{ padding: '7px 12px', textAlign: 'right',
                                    color: (m?.cold_pixel_count || 0) > 0 ? t.accent : t.textMuted }}>
                        {m?.cold_pixel_count ?? '—'}
                      </td>,
              };
              return (
                <tr key={idx} style={{ background: idx % 2 === 0 ? t.panel : t.panelAlt,
                                        borderBottom: `1px solid ${t.border}` }}>
                  {cols.map(c => <React.Fragment key={c.key}>{cellByKey[c.key]}</React.Fragment>)}
                </tr>
              );
            })}
            {sorted.length === 0 && (
              <tr><td colSpan={cols.length} style={{ padding: 40, textAlign: 'center', color: t.textFaint }}>
                No rows.
              </td></tr>
            )}
          </tbody>
...
// ---------------------------------------------------------------------------
const FPNHistogramsTab = ({ channels, measurements, visibleRoiIdx, roiLabel,
                             unit = 'DN', fullDR = 65535 }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  ...
  return (
    <div>
      <div data-no-export style={{ fontSize: 11, color: t.textMuted, marginBottom: 10,
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 14 }}>
        {channels.flatMap(ch =>
          visibleRoiIdx.map(i => {
            const m = measurements[ch]?.[i];
            return (
              <FPNHistChart key={`${ch}-${i}`} channel={ch} roiName={roiLabel(i)}
                            measurement={m} unit={unit} fullDR={fullDR} />
            );
          })
        )}
      </div>
...
const FPNHistChart = ({ channel, roiName, measurement, unit, fullDR }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const W = 320, H = 180, PAD_L = 40, PAD_R = 8, PAD_T = 14, PAD_B = 28;
  const color = channelColor(channel);
  const xfmt = (v) => unit === 'pctDR' ? `${(v / fullDR * 100).toFixed(1)}%` : v.toFixed(0);
  if (!measurement?.hist_counts || !measurement?.hist_bin_edges) {
    return <div style={{ background: t.panel, border: `1px solid ${t.border}`, borderRadius: 8, padding: 12, color: t.textFaint, textAlign: 'center', minHeight: H + 30 }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: t.text, marginBottom: 6,
                    fontFamily: 'ui-monospace,Menlo,monospace', textAlign: 'left' }}>
        {channel} · {roiName}
      </div>
    ...
    <div style={{ ...cardChromeFor(style, t) }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
        ...
        <span style={{ fontSize: 12, fontWeight: 600, color: t.text,
                       fontFamily: 'ui-monospace,Menlo,monospace' }}>{channel} · {roiName}</span>
        <span style={{ flex: 1 }} />
        <span style={{ fontSize: 10, color: t.textFaint, fontFamily: 'ui-monospace,Menlo,monospace' }}>
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%" style={{ display: 'block', background: 'transparent', borderRadius: 4 }}>
        ...
        {counts.map((c, i) => {
          ...
          const y  = yToPx(c);
          return <rect key={i} x={x0} y={y} width={Math.max(0.5, x1 - x0)}
...
const FPNProfilesTab = ({ channels, measurements, rois, visibleRoiIdx, roiLabel }) => {
  ...
  return (
    <GridTabFrame
      ...
      n={cards.length} minCardPx={340}
      storageKey="analysis/fpn/profilesLayout">
      {cards.map(({ i, ch, m, label }) => <RowColCard key={`${ch}_${i}`}
        ch={ch} label={label} m={m} />)}
    </GridTabFrame>
  );
};
...
const RowColCard = ({ ch, label, m }) => {
  ...
  const plot = (label, values, stds) => {
    ...
    const mid = (lo + hi) / 2, range = Math.max(1e-9, hi - lo) * 1.15;
    const yMin = mid - range / 2, yMax = mid + range / 2;
    const N = vs.length;
    const xOf = (i) => PAD_L + (i / Math.max(1, N - 1)) * (W - PAD_L - PAD_R);
    const yOf = (v) => PAD_T + (1 - (v - yMin) / (yMax - yMin)) * (H - PAD_T - PAD_B);
    const pts = vs.map((v, i) => Number.isFinite(v) ? `${xOf(i).toFixed(2)},${yOf(v).toFixed(2)}` : null)
                  .filter(Boolean).join(' ');
    // ±σ band (stds is same length)
    const bandPts = [];
    if (stds?.length) {
      for (let i = 0; i < N; i++) {
        const v = vs[i], s = stds[i];
  ...
  return (
    <div style={{ background: t.panel, border: `1px solid ${t.border}`,
      ...
      <div style={{ fontSize: 9.5, color: t.textFaint, marginTop: 4,
                    fontFamily: 'ui-monospace,Menlo,monospace', display: 'flex',
...
const PSD1DChart = ({ axis, channels, roiIdx, measurements }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const W = 340, H = 120, PAD_L = 36, PAD_R = 10, PAD_T = 10, PAD_B = 24;
  const key = axis === 'row' ? ['row_freq', 'row_psd', 'row_peak_freq']
                             : ['col_freq', 'col_psd', 'col_peak_freq'];
  // Collect per-channel (freq, psd) drops DC
  const series = channels.map(ch => {
    const m = measurements[ch]?.[roiIdx]; if (!m || m.error) return null;
  ...
  for (const s of series) for (const v of s.p) {
    const l = Math.log10((v || 1e-12) + 1e-12);
    if (l < lo) lo = l; if (l > hi) hi = l;
  }
  const range = Math.max(1e-9, hi - lo);
  const xToPx = (fx) => PAD_L + (fx / 0.5) * (W - PAD_L - PAD_R);
  const yToPx = (pv) => PAD_T + (1 - (Math.log10((pv || 1e-12) + 1e-12) - lo) / range) * (H - PAD_T - PAD_B);
  return (
    <div>
      <div style={{ fontSize: 10, color: t.textFaint, marginBottom: 2, marginTop: 4,
                    fontFamily: 'ui-monospace,Menlo,monospace' }}>
      ...
      <div style={{ display: 'flex', gap: 10, marginTop: 3, flexWrap: 'wrap' }}>
        {series.map(s => (
          <span key={s.ch} style={{ fontSize: 9.5, color: channelColor(s.ch),
                                     fontFamily: 'ui-monospace,Menlo,monospace' }}>
...
const HotColdList = ({ m }) => {
  ...
  if (!hot.length && !cold.length) {
    return <div style={{ fontSize: 10.5, color: t.success, textAlign: 'center', padding: '4px 0' }}>
      No outliers at the current |z| threshold.
    </div>;
  }
  const renderList = (list, color, label) => (
    <div>
      <div style={{ fontSize: 10, color: t.textMuted, textTransform: 'uppercase', letterSpacing: 0.4, marginBottom: 2 }}>
        {label} <span style={{ color }}>({list.length})</span>
      </div>
      <div style={{ maxHeight: 100, overflow: 'auto', fontFamily: 'ui-monospace,Menlo,monospace',
...
const FPNCompareTab = ({ channels, measurements, rois, visibleRoiIdx, roiLabel }) => {
  ...
  return (
    <div>
      ...
      <div style={{ display: 'grid',
                    gridTemplateColumns: 'repeat(auto-fill, minmax(420px, 1fr))', gap: 14 }}>
        {metrics.map(mm => (
          <MetricBars key={mm.key}
            metric={mm} channels={channels}
...
const MetricBars = ({ metric, channels, measurements, visibleRoiIdx, roiLabel }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const W = 400, H = 220, PAD_L = 40, PAD_R = 14, PAD_T = 10, PAD_B = 50;
  // Collect values
  const rows = visibleRoiIdx.map(i => channels.map(ch => {
    const m = measurements[ch]?.[i];
        ...
        {visibleRoiIdx.map((i, gi) => (
          <g key={i} transform={`translate(${PAD_L + gi * groupW}, 0)`}>
            {channels.map((ch, ci) => {
              ...
              return (
                <g key={ch}>
                  <rect x={x} y={y} width={barW - 1} height={H - PAD_B - y}
...
const DoFAnalysisModal = ({ run, onClose, onToast }) => {
  ...
  const exportCSVRows = useCallbackA(() => {
    ...
    for (const ch of visibleChannels) {
      ...
      (r.lines || []).forEach((ln, i) => {
        ...
        const g = ln.gaussian || {};
        rows.push({
          channel: ch, kind: 'line',
          line_idx: i, label: lineLabel(i),
          p0_x: ln.p0?.[0], p0_y: ln.p0?.[1], p1_x: ln.p1?.[0], p1_y: ln.p1?.[1],
          peak_position_px: ln.peak_position_px?.toFixed?.(4),
          dof_low_px: ln.dof_low_px, dof_high_px: ln.dof_high_px,
      ...
      (r.points || []).forEach((pt, i) => {
        rows.push({
          channel: ch, kind: 'point',
          point_idx: i, label: pointLabel(i),
          x: pt.x, y: pt.y,
  ...
  const exportJSONRaw = useCallbackA(() => {
    exportJSON(`mantis-dof-${Date.now()}.json`, {
      kind: 'mantis-dof-analysis', version: 1, exportedAt: new Date().toISOString(),
      channels: visibleChannels, lines: allLines, points: allPoints,
      settings,
      display: { unit_pref: unitPref, tilt_angle_deg: tiltAngleDeg,
                 tilt_factor: tiltFactor },
      results: Object.fromEntries(visibleChannels.map(ch => [ch, results[ch] ?? null])),
    });
    onToast?.('Exported analysis JSON', 'success');
  }, [visibleChannels, allLines, allPoints, settings, results, onToast,
      unitPref, tiltAngleDeg, tiltFactor]);
    ...
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,12,18,0.62)',
      ...
      <div onClick={(e) => e.stopPropagation()}
        ...
        <div style={{ padding: '14px 18px', borderBottom: `1px solid ${t.border}`,
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 14.5, fontWeight: 600, color: t.text }}>
              DoF analysis — {visibleChannels.length} channel{visibleChannels.length !== 1 ? 's' : ''} × {allLines.length} line{allLines.length !== 1 ? 's' : ''} · {allPoints.length} point{allPoints.length !== 1 ? 's' : ''}
              {tiltFactor !== 1 && (
                <span style={{ marginLeft: 8, fontSize: 11, fontWeight: 500,
                               color: t.warn, fontFamily: 'ui-monospace,Menlo,monospace' }}>
            ...
            <div style={{ fontSize: 11, color: t.textFaint,
                          fontFamily: 'ui-monospace,Menlo,monospace', marginTop: 1 }}>
              metric={settings.metric} · half-win={settings.half_window}px · threshold={(settings.threshold * 100).toFixed(0)}%
          ...
          <PlotStylePanel open={false} onToggle={() => setStyleOpen(s => !s)} />
          <Button icon="image"    onClick={exportPNG}     title="Capture the active tab as a PNG">PNG</Button>
          <Button icon="download" onClick={exportCSVRows} title="Flat per-(channel × line/point) CSV">CSV</Button>
          <Button icon="download" onClick={exportJSONRaw} title="Round-trippable raw analysis JSON">JSON</Button>
          <Button variant="subtle" icon="close" onClick={onClose} size="sm" title="Close (Esc)" />
        </div>
        {styleOpen && <PlotStylePanel open={true} onToggle={() => setStyleOpen(false)} />}
        ...
        <div style={{ padding: '10px 18px', borderBottom: `1px solid ${t.border}`,
          ...
          <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
                         letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>Line</span>
          <Segmented value={lineIdxFilter} onChange={setLineIdxFilter} options={[
            { value: 'all', label: 'All' },
            ...allLines.map((_, i) => ({ value: String(i), label: lineLabel(i) })),
          ]} />
          {settings.compute_all_metrics && <>
            <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
                           letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>Metric</span>
            <Segmented value={metricFilter} onChange={setMetricFilter} options={[
          ...
          {unitPref !== 'auto' && unitPref !== 'px' && !anyCalibrated && (
            <span title="No line is calibrated — values will fall back to pixels."
                  style={{ fontSize: 10, color: t.warn,
                           fontFamily: 'ui-monospace,Menlo,monospace' }}>
              calibration missing
            </span>
          )}
          <span style={{ fontSize: 10, color: t.textMuted, fontWeight: 600,
                         letterSpacing: 0.5, textTransform: 'uppercase', marginLeft: 6 }}>Tilt</span>
          <input type="number" min={0} max={89} step={0.5}
                 value={tiltAngleDeg}
                 ...
                 onChange={(e) => {
                   ...
                   if (!Number.isFinite(v)) { setTiltAngleDeg(0); return; }
                   setTiltAngleDeg(Math.min(89, Math.max(0, v)));
                 }}
                 style={{ width: 60, padding: '4px 6px', fontSize: 12,
                          fontFamily: 'ui-monospace,Menlo,monospace',
                          background: t.panel, border: `1px solid ${t.border}`,
                          borderRadius: 4, color: anyCalibrated ? t.text : t.textFaint,
        ...
        <div ref={tabBodyRef} style={{ flex: 1, minHeight: 0, overflow: 'auto', padding: 16,
          ...
          {tab === 'lines' && <DoFLinesTab channels={visibleChannels} results={results}
                                             visibleLineIdx={visibleLineIdx}
...
const dofDefaultDigits = (unit) =>
  unit === 'px' ? 2 : unit === 'μm' ? 1 : unit === 'mm' ? 3 : 4;
const dofFmt = (ln, px, prefOrDigits, maybeDigits) => {
  ...
  const dPx = typeof prefOrDigits === 'number' ? prefOrDigits
             : typeof maybeDigits === 'number' ? maybeDigits : 2;
...
const DoFSummaryTab = ({ channels, results, visibleLineIdx, lineLabel, pointLabel,
  ...
  const rows = useMemoA(() => {
    ...
    for (const ch of channels) {
      ...
      (r.lines || []).forEach((ln, i) => {
        if (!visibleLineIdx.includes(i)) return;
        out.push({ order: out.length, kind: 'line', ch, i, ln,
                   label: lineLabel(i), g: ln.gaussian || {} });
      });
    }
    return out;
  ...
  const allSameDisp = displayedUnits.length > 0
    && displayedUnits.every(u => u === displayedUnits[0]);
  const unitLabel = allSameDisp
    ? displayedUnits[0]
    : (allHaveUnit ? rows[0].ln.unit_name : 'px');
  const cols = [
    { key: 'channel',  label: 'Channel',           align: 'left' },
  ...
  return (
    <div style={{ background: t.panel, border: `1px solid ${t.border}`,
      ...
      <div style={{ padding: '10px 14px', borderBottom: `1px solid ${t.border}`,
        ...
        <span>{sorted.length} rows · {channels.length} channels × {visibleLineIdx.length} lines</span>
        <span style={{ flex: 1 }} />
        <span style={{ fontSize: 10.5 }}>click a header to sort</span>
      </div>
      <div style={{ maxHeight: 'calc(94vh - 320px)', overflow: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse',
          ...
          <tbody>
            {sorted.map((row, idx) => {
              ...
              const r2Color = r2 == null ? t.textFaint
                            : r2 >= 0.9 ? t.success
              ...
              return (
                <tr key={idx} style={{ background: idx % 2 === 0 ? t.panel : t.panelAlt,
                  ...
                  <td style={{ padding: '7px 12px', color: chColor, fontWeight: 600,
                               display: 'flex', alignItems: 'center', gap: 6 }}>
                    <span style={{ width: 7, height: 7, borderRadius: '50%', background: chColor }} />{row.ch}
                  </td>
                  <td style={{ padding: '7px 12px', color: t.text }}>{row.label}</td>
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: t.text }}>
                    {dofFmt(row.ln, dofScaled(row.g.converged ? row.g.mu : row.ln.peak_position_px, tiltFactor), unitPref, 2)}
                  </td>
                  ...
                  <td style={{ padding: '7px 12px', textAlign: 'right', color: r2Color, fontWeight: 600 }}>
                    {r2?.toFixed?.(3) ?? '—'}
                  </td>
                </tr>
              );
...
// ---------------------------------------------------------------------------
const DoFLinesTab = ({ channels, results, visibleLineIdx, lineLabel, unitPref = 'auto',
                        tiltFactor = 1 }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  ...
  return (
    <GridTabFrame
      ...
      n={visibleLineIdx.length} minCardPx={420}
      storageKey="analysis/dof/linesLayout">
      {visibleLineIdx.map(i => (
        <LineOverlayChart key={i} idx={i} channels={channels} results={results}
                           label={lineLabel(i)} unitPref={unitPref}
                           tiltFactor={tiltFactor} />
      ))}
    </GridTabFrame>
  );
...
const LineOverlayChart = ({ idx, channels, results, label, unitPref = 'auto',
                             tiltFactor = 1 }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const W = 420, H = 260, PAD_L = 44, PAD_R = 14, PAD_T = 14, PAD_B = 36;
  ...
  const xMin = 0;
  const xMax = Math.max(...series.map(s => {
    const xs = xsOf(s);
    return xs[xs.length - 1];
  }));
  const xOf = (x) => PAD_L + ((x - xMin) / (xMax - xMin || 1)) * (W - PAD_L - PAD_R);
  const yOf = (y) => PAD_T + (1 - Math.max(0, Math.min(1, y))) * (H - PAD_T - PAD_B);
  const xTicks = [xMin, xMin + (xMax - xMin) / 4, xMin + (xMax - xMin) / 2,
                   xMin + 3 * (xMax - xMin) / 4, xMax].map(v => Math.round(v));
  const yTicks = [0, 0.25, 0.5, 0.75, 1];
          ...
          return (
            <g key={ch}>
              {lr.dof_low_px != null && lr.dof_high_px != null && (
                <rect x={xOf(toAxisX(lr.dof_low_px))} y={PAD_T}
...
// ---------------------------------------------------------------------------
const DoFMetricCompareTab = ({ channels, results, visibleLineIdx, lineLabel,
                                unitPref = 'auto', tiltFactor = 1 }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const cards = [];
  for (const i of visibleLineIdx) for (const ch of channels) {
...
const MetricOverlayChart = ({ ch, lr, label, unitPref = 'auto', tiltFactor = 1 }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const W = 380, H = 220, PAD_L = 40, PAD_R = 12, PAD_T = 14, PAD_B = 30;
  const METRIC_COLORS = {
    laplacian: '#1f77b4', brenner: '#d62728',
    ...
    <ChartCard ch={ch} sub={`· ${label}`}
      ...
      <svg viewBox={`0 0 ${W} ${H}`} width="100%"
        ...
        {['laplacian', 'brenner', 'tenengrad', 'fft_hf'].map(m => {
          ...
          const peakAxis = msw.peak_position_px != null
                         ? toAxis(msw.peak_position_px) : null;
          return (
            <g key={m}>
...
const ChromaticShiftChart = ({ channels, results, visibleLineIdx, lineLabel,
                                unitPref = 'auto', tiltFactor = 1 }) => {
  const t = useTheme();
  const { style } = usePlotStyle();
  const W = 400, H = 260, PAD_L = 50, PAD_R = 16, PAD_T = 14, PAD_B = 40;
...
const LegacyPngModal = ({ run, onClose, onToast }) => {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(10,12,18,0.62)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 80, padding: 24 }} onClick={onClose}>
      <div onClick={(e) => e.stopPropagation()} style={{ width: '100%', maxWidth: 1400, height: '92vh', background: t.panel, border: `1px solid ${t.border}`, borderRadius: 10, boxShadow: t.shadowLg, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ padding: '14px 18px', borderBottom: `1px solid ${t.border}`, display: 'flex', alignItems: 'center', gap: 12 }}>
          <div style={{ width: 28, height: 28, borderRadius: 6, background: t.accentSoft, color: t.accent, display: 'flex', alignItems: 'center', justifyContent: 'center' }}><Icon name="run" size={14} /></div>
          <div style={{ flex: 1 }}><div style={{ fontSize: 14, fontWeight: 600, color: t.text }}>{title}</div></div>
          <Button variant="subtle" icon="close" onClick={onClose} size="sm" />
        </div>
        <div style={{ display: 'flex', borderBottom: `1px solid ${t.border}`, background: t.panelAlt, padding: '0 8px', gap: 2 }}>
          {tabs.map((x, i) => (
            <button key={x.key} onClick={() => setTab(i)} style={{
              padding: '10px 14px', fontSize: 12, fontWeight: tab === i ? 600 : 450,
              background: 'transparent', border: 'none', cursor: 'pointer',
        ...
        <div style={{ flex: 1, overflow: 'auto', padding: 16, background: t.panelAlt }}>
          {current.png
            ? <img src={`data:image/png;base64,${current.png}`} alt={current.label} style={{ maxWidth: '100%', maxHeight: 'calc(92vh - 180px)', borderRadius: 6, background: '#fff' }} />

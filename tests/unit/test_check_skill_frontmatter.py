...
def test_valid_skill_passes(tmp_path: Path) -> None:
    ...
    p = _write_skill(tmp_path, "good-skill", (
        "name: good-skill\n"
...
def test_name_directory_mismatch_flagged(tmp_path: Path) -> None:
    mod = _load(tmp_path)
    p = _write_skill(tmp_path, "dir-name", (
        "name: wrong-name\n"
        "description: x\n"
        "when_to_use:\n  - trig"
    ))
...
def test_description_too_long_flagged(tmp_path: Path) -> None:
    ...
    p = _write_skill(tmp_path, "long-desc", (
        f"name: long-desc\n"
...
def test_broken_related_agent_flagged(tmp_path: Path) -> None:
    mod = _load(tmp_path)
    p = _write_skill(tmp_path, "broken-ref", (
        "name: broken-ref\n"
        "description: x\n"
        "when_to_use:\n  - trig\n"
...
def test_empty_when_to_use_flagged(tmp_path: Path) -> None:
    mod = _load(tmp_path)
    p = _write_skill(tmp_path, "empty-triggers", (
        "name: empty-triggers\n"
        "description: x\n"
        "when_to_use:\n"
    ))
